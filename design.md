...
The shell also carries the desktop client's terminal renderer capability. chan-desktop decides whether its WebKit process can use the accelerated path, appends `chan-renderer=webgl|dom` to every window URL it builds, and the serving tenant injects the last recognized value as `<meta name="chan-webgl-renderer" content="1|0">`. Carrying the answer on the URL reaches embedded, outbound, and tunneled tenants even when the server runs on another machine. The desktop appends its value last so its live decision wins over a value already present in an attachment URL; every shell response is `Cache-Control: no-store`, so the signal is not reused across window loads.

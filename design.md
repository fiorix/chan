...
## The standalone Files application

`?kind=files` is a third window mode beside workspace and standalone terminal: a Files window rides the SAME shared standalone terminal tenant (storage kind `terminal` plus an `app: "files"` discriminator on its window record, so an older reader degrades it to a Terminal row) and hosts the File Browser, the editor, and terminals over the server machine's filesystem with no workspace behind them. The filesystem face is `chan_workspace::MiniWorkspace`: a `/`-rooted capability with canonical `$HOME` as the protected start directory, no registry row, no metadata, no index, no graph, symlinks inert, deletes limited to files and empty directories, plain moves and copies with whole-tree preflight. The slim tenant mounts the Files route surface (`/api/fs/context`, per-directory listing, streamed reads, CAS writes, create/delete/move/transfer, uploads and attachments) beside its terminal routes; the `cs upload`/`cs download` transfer lane is unchanged and shares the `GET /api/files/{*path}` and `POST /api/files/upload` paths by in-handler dispatch. Live refresh is a scoped watch manager owning one NON-recursive OS watch per subscribed directory (never a recursive watch of `/`), driven by the `/ws` scope registry's refcount deltas, with typed `fs_reset` frames (`subscribed`, `watch_error`, `overflow`, `directory_replaced`) demanding a one-level relist. A Files window's own mutations are attributed (`source_w` on the scoped `fs` frame) through a begin/commit/cancel ticket bus instead of the workspace tenants' drop-the-echo suppression, so sibling Files windows relist while the writer's clean buffers stay quiet; a write through Files is an external write to any real Workspace covering the same file. Doc and scene authorities never engage on this tenant. The capability is advertised as `files_app` on `DevserverInfo` and gated at mint server-side. The launcher offers no Files action of its own -- Files windows list among that machine's standalone terminal rows, to hide, reopen and close like any other window -- because the way in is `cs window new`, which spawns another window of whatever the CALLING window (`$CHAN_WINDOW_ID`) runs: a Files window from a Files window, a terminal from a terminal, another workspace window from a workspace. That id is what separates them, since a Files window's shells ride the same shared terminal tenant as a plain standalone terminal's; an absent or unresolvable id degrades to the tenant's default window rather than failing.

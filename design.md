...
On Linux chan does not blindly overwrite whatever unit already sits at `chan-devserver.service`. It classifies the installed unit against its own renderer as `Current`, `KnownLegacy`, or `Foreign` (`DevserverUnitClass` in `crates/chan-systemd/src/lib.rs`): a `Current` unit that already matches is left untouched, a `Foreign` (administrator-edited or non-chan) unit is refused with an actionable error -- move or remove it, then retry -- rather than clobbered, and a `KnownLegacy` chan-owned shape is migrated in place (rewrite the unit, `daemon-reload`, `restart`), rolling the previous unit back on failure and preserving the fdstore ownership so live terminal PTYs survive the bounce. The unit pins `WatchdogSec=30` and the devserver answers with a `WATCHDOG=1` ping at half that interval, so a seized-but-alive process trips systemd's watchdog restart while a healthy one keeps the timer fed.

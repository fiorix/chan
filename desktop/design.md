...
Devserver windows do not load the remote URL directly: a down remote would paint a blank white webview (WKWebView never finishes navigating). They load a bundled local connecting/retry page instead, which shows the attempt log, probes the remote through the `probe_url` IPC, and on success navigates the same window to the fully assembled tenant URL with its `?w=` and `?lib=` identity. Only that trusted local page may call `probe_url`; the command is not in the grant for gateway-served `lib-*` windows. A loopback target treats any HTTP response as reachable; a gateway target retries on 502, 503, 504, and transport failures. The Rust request carries the target origin's webview cookies when available so the probe can distinguish a registered-but-not-answering gateway devserver from a live one. The page cannot probe the remote itself because the strict CSP blocks cross-origin fetches, so Rust owns the per-attempt timeout. Cmd/Ctrl+W and the close button on the connecting screen cancel and really close.

import { isFilesWindow } from "../api/client";
...
export function terminalWsPath(opts: TerminalWsPathOpts): string {
  ...
  if (sessionId) {
    ...
    // A Files window's cwd is a wire-relative path the server must resolve
    // through the standalone capability root, not the workspace resolver;
    // the marker rides only fresh spawns (a reattach keeps its PTY cwd).
    if (isFilesWindow()) params.set("app", "files");

...
A devserver publishes itself with nothing but an outbound connection and a PAT; no inbound port, DNS, or TURN/STUN stack is involved. The PAT is the only credential the devserver holds: the devserver id is the lowercase hex SHA-256 of the raw PAT, so one token identifies one devserver. `chan_tunnel_proto::gateway_assertion::devserver_id_from_token` derives that id. The id is the tunnel registry's second key and the value carried by the entry credential's `drv` claim. It encodes the same SHA-256 digest that identity stores in `api_tokens.token_hash`, which uses unpadded base64url instead. Identity emits a newly minted plaintext PAT only in the one-time SPA/admin creation response or desktop redemption response and persists only its digest. A configured devserver presents its tunnel PAT once per dial in the tunnel Authorization header and periodically for the life of the tunnel in lease-refresh frames. devserver-proxy sends the PAT to identity for dial validation, for up to three Hello-name announcement attempts, and for every lease refresh. While a tunnel is connected, the proxy process also keeps the dial PAT in h2's HPACK decoder for the life of that h2 connection; see `crates/chan-tunnel-server/design.md` for the detail. The client requires HTTPS to the tunnel URL except when its cleartext TCP peer is verified as loopback. The proxy's identity URL must use HTTPS or literal loopback HTTP unless the deployment declares an authenticated encrypted overlay; `IDENTITY_INTERNAL_TOKEN` authenticates but does not encrypt those internal requests. chan-desktop separately presents its stored desktop PAT directly to identity in the Authorization header on every roster request and entry mint.
...
3. **Admission.** The proxy admits nothing unless its control session is at `FleetReady`. devserver-control re-verifies the lease signature and binding, then decides: `Admit`, `AtCapacity` (the per-owner connected-devserver cap is `min(MAX_DEVSERVERS_PER_USER, the signed policy value)`; admission also reserves per-session and fleet rows plus serialized fleet bytes for pending claims), `Stale`, or `ControlWarming`. Refusals reach the client as `HelloAck::Refused` with a machine-readable code.

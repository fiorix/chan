...
Connection hygiene: the accept loop follows the shared accept-failure policy ([`chan-tunnel-proto/design.md`](../../../crates/chan-tunnel-proto/design.md#accept-failures)): a failure that concerns one pending connection is retried at once, one that means the process is out of descriptors or memory (or is unrecognised) is logged at error and retried after one second, and only a failure that means the listening socket is unusable ends the listener, and with it the process, since this is the fleet's only controller. The admin listener (axum) already pauses and retries the same way. Each connection runs in its own task, and a panic in one (the gateway release profile unwinds) ends only that proxy's session: the listener logs it at error with the peer and keeps serving, the unwind releases the connection's slot, and the actor retires the session the next time it sends it a frame. The h2 handshake, first stream, and `ClientHello` each have a 10s deadline; every write to a proxy (`ServerHello`, each command, a resync, and a final `Shutdown` with the half-close after it) is bounded by `SESSION_DEAD_AFTER` (15s), because an h2 write stays pending until the proxy grants flow-control window, and a write parked in a session's `select!` arm stops that session's deadlines from being polled, so a proxy that stops reading would otherwise keep its connection task and in-flight slot after the actor retired its session. A write that times out ends the session, a `Shutdown` included; the initial or resync snapshot has an absolute 30s deadline; at most 128 connections are in flight; a connection that opens extra streams gets 409 per stream and is shut down after 16 of them. One framed-reader task owns the inbound side because a length-prefixed read is not cancellation-safe mid-frame. Its queue is 64 frames and the established session accepts at most 32 frames in any one-second sliding window. A full maximum tunnel snapshot needs only 18 frames, so a compromised authenticated proxy is disconnected before it can continuously monopolize the shared actor queue.
...
The session remembers registrations the controller removes before their proxy publishes a down: lease expiry, a kill reported `killed` or `missing`, and refused or unclaimed publications that never enter the session rows. Refused refreshes are remembered while their rows await removal. A kill reported `missing` can overtake a down already queued at the proxy; if that down was already consumed, the result remembers nothing. One later `TunnelDown` for a remembered id advances the generation and forgets the id, and queued `LeaseRefresh` frames for it are dropped. A second down, an arbitrary unknown down, or a remembered id published up again still resyncs; a refresh for an unknown id still ends the session. The shared record holds at most 4,096 ids, clears on resync, and starts empty in a new session. Past the bound new removals are not remembered: their down resyncs and a refresh for their removed row ends the session.
...
- The session adds each refused registration id to the shared removal record from the moment of refusal. The proxy's `TunnelDown` for it is accepted and advances the generation whether it arrives after the kill's result or before it (a tunnel that closed on its own before the kill landed, which the kill then reports missing). A refresh for it that raced the kill is dropped. The shared bound and overflow behavior apply.
...
Safety caps also bound admission: live plus pending rows must fit the asking session's 2,048-row limit and the fleet's 16,384-row limit. Fleet resident bytes plus a conservative maximum serialized tunnel row for every pending claim, including the new claim, must fit 64 MiB; each reservation is 4,866 bytes, covering the up-to-4,096-byte verified admission lease, at most 2x JSON escaping for verified names, and all remaining fields. The byte cap therefore allows 13,791 pending claims on an empty fleet, below the 16,384-row cap because that many maximum-size verified rows would exceed 64 MiB. A `TunnelUp` that nevertheless exceeds a session or fleet row/byte cap releases its matching claim and commands only that registration killed. Its later `TunnelDown` is expected, and the session and its other rows remain active. A `LeaseRefresh` that would grow a row past the fleet byte cap uses the same registration refusal; the row keeps its previous lease until the kill or down removes it. Snapshot limits still reject the session.
...
Snapshots can disagree with the aggregate: two proxies may report rows for the same `(user, devserver_id)` key, or a snapshot may push a user over capacity. Reconciliation picks winners and commands the losers down before the new view becomes visible. Only one reconciliation runs at a time. A joining snapshot that arrives while the controller is ready and another reconciliation runs is accepted and deferred: the proxy receives `SnapshotAccepted`, its session stays Joining with its rows out of the aggregate, it keeps taking deltas and session revocations, and its own join starts when the running reconciliation finishes, in arrival order. A deferred session that disconnects or is force-resynced leaves the queue, and a resync snapshot queues it again. If the controller leaves readiness while a join waits, the waiting session becomes Active the way any snapshot does on an unready controller and joins initial reconciliation. A deferred proxy still owes `FleetReady` within its 45-second convergence deadline; a queue longer than that makes it evict its local tunnels fail-closed, not lose its session.

**Routine join (live-first).** While the controller is ready, a joining snapshot reconciles against the live aggregate, and every live row is an immutable winner: those rows were admitted during this controller lifetime, so their recency is known and a joining snapshot must never outrank it. Joining rows that duplicate a live key lose, each user's live rows are reserved against the capacity limit first, and only novel keys that fit the remaining slots are admitted. A key with more than one row in a joining snapshot is a conflict, and every row in the group loses, as in initial reconciliation. Neither proxy nor registration id establishes recency among conflicting rows. If any loser kill fails or times out, the joining session is removed and its proxy reconnects and retries the whole join.
...
- **Retained disconnected authority.** A disconnected proxy's tunnel and tenant-session rows remain visible and continue to consume row, byte, and capacity authority through the 30-second row convergence window. Its `(proxy_id, boot_id)` authority marker remains for 60 seconds after the latest disconnect, which exceeds the proxy's longest 45-second retained-authority path. Every same-boot disconnect extends the marker, and only successful same-boot convergence through `FleetReady` clears it early. Admin session revocation reports that proxy unreachable while this marker exists and no session from the same boot is connected. A reconnected same-boot session receives revocations from the moment it begins, whether its join is reconciling, deferred, or done, so while it is connected the proxy is reachable rather than unreachable, and if it disconnects before `FleetReady` the marker is still there, extended.
...
| `FleetCapacity`                   | session | snapshot cap or invalid revocation count   |

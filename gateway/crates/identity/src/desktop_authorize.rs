...
/// code. Alongside `CreatedToken` and the `CreatedTokenView` shared by
/// `POST /api/tokens` and operator `POST /admin/v1/tokens`, this is one
/// of only three serialized identity response types that carry a
/// plaintext PAT.

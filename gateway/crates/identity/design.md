...
- Revoke (`DELETE /api/tokens/{id}`):
  ...
  - Identity makes a best-effort immediate owner-tunnel/session cut and returns `202`; profile's worker confirms a post-commit first cut and a second fleet cut after the full entry-credential quiet window. Identity does not evict the revoked PAT's tunnels alone: the revoke path calls `kill_owner_tunnels`, which cuts every tunnel the owner has. Registrations carry the digest-derived devserver id, not the token id, and that devserver id is enough to target one PAT's tunnel because `devserver-control` exposes `POST /admin/v1/tunnels/{owner_user_id}/{devserver_id}/kill` and identity is authorized to call it. Identity could derive the devserver id from the token (it is the same SHA-256 digest stored as `api_tokens.token_hash`), but it does not today, so the conservative scope remains the whole subject.

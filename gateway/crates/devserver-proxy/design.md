...
The listener generates the registration UUID. A lease the proxy cannot verify is refused with `control_unavailable` without reaching the controller, which would otherwise reserve capacity for a tunnel this proxy could never publish. Otherwise the control session sends the lease and exact registration tuple to devserver-control and waits. Only an `Admit` decision permits `HelloAck::Ok` and registry insertion. `AtCapacity` maps to `too_many_workspaces`; warming, stale, or unavailable authority maps to `control_unavailable`. There is no local admission fallback.

...
- **The proxy plane**: each proxy node runs devserver-proxy behind its own TLS edge. That edge serves the `proxy.<domain>` tunnel apex (`/v1/tunnel` is negotiated as h2 externally and `grpc_pass`ed as h2c into the proxy's tunnel listener, on a location that needs the raised idle timeouts under [The TLS edges](#the-tls-edges)) and the node's tenant wildcard `*.{proxy}.proxy.<domain>` (ordinary HTTP plus WebSocket upgrade). The node connects back to the plane above over the authenticated overlay, and can reach exactly two things there: devserver-control's proxy-control listener and identity's internal token-validate listener. Nothing else (postgres, profile, identity's public listener) is reachable from a proxy node; production treats proxy nodes as compromisable, and `packaging/kube/network-policy.yaml` encodes the same reachability contract for clusters.
...
The tunnel location needs idle timeouts that fit a tunnel. A registered tunnel is one long-lived `POST /v1/tunnel` whose request body is the devserver-to-proxy direction, and that body is idle for long stretches when nothing is using the devserver, so nginx's `client_body_timeout` (60s by default) expires and ends it: no error on either side, a `200` in the access log, and a devserver that re-registers about once a minute forever. Raise `client_body_timeout` on that location: it is the deadline that ends an idle tunnel under an otherwise default configuration. `grpc_read_timeout` is an idle deadline on the same request's upstream leg, which is quiet for the same stretches, and nginx ends the request with a `504` and an `upstream timed out` line in the error log when it expires, so raise it alongside. `grpc_send_timeout` is armed only while a write to the upstream is still pending, so it is not an idle deadline on a tunnel. [`nginx-edge.sh`](../../packaging/gateway/scripts/dev/sdme/devserver-tunnel-e2e/nginx-edge.sh) in the tunnel e2e rig renders the shape.

Both edges decode a bearer they then forward. The tunnel edge decodes every dial's `Authorization: Bearer <PAT>` header; the identity edge decodes the desktop PATs that `/desktop/v1/*` carries. In nginx 1.28.3 the decoded header field is allocated out of the HTTP/2 stream's memory pool, which for a tunnel lives as long as the tunnel does, and that pool is freed without being cleared, so the plaintext stays in the worker's heap until the worker reuses the block. The detail is nginx's; the property is not, and any TLS-terminating edge holds the bearers it decodes. Isolate an edge host the way the gateway pod is isolated, and keep PAT rotation and revocation as the control. Section 6 of [`crates/chan-tunnel-server/design.md`](../../crates/chan-tunnel-server/design.md) says what a captured tunnel PAT reaches.

...
export default {
  ...
  async run(ctx) {
    ...
    try {
      ...
      await ctx.shot("after-first-open", page);
      ...
      record("after-merge-window", {
        ...
        editorShows: editorAfter?.includes(V2)
          ...
          : editorAfter?.includes(V1)
            ? "v1"
            : "neither",
        diskShows: diskAfter.includes(V2)
          ? "v2"
          : diskAfter.includes(V1)
            ? "v1"
            : "neither",
      });
      await ctx.shot("after-external-edit", page);
      ...
      const last = evidence.steps.find(
        (s) => s.step === "after-second-cs-open",
      );

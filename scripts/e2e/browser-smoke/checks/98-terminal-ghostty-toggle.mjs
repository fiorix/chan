...
export default {
  ...
  async run(ctx) {
    ...
    /// Flip one terminal.* preference via the exact settings-UI shape:
    /// a serialized in-page GET /api/config -> mutate the terminal slice
    /// preserving siblings -> whole-block PATCH (updateGlobalConfigSerial's
    /// contract, state/configWrite.ts). Mirrors 97's setMouseCapture.

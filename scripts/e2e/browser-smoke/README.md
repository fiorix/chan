...
Files under `checks/` run in sorted filename order. The sort is LEXICAL, not numeric: `100-*` and `110-*` run right after `10-*`, and the true tail slots are `96`/`97`/`98` (after `95-*`). Pick a number with the lexical order in mind. Each default-exports `{ name, run(ctx) }`; `run` throws (or returns) and may record intermediate evidence:

...
- The guest installs Ubuntu `nix-bin`, evaluates the repository flake, builds the selected package, and runs the existing package smoke.
- `NIX_PACKAGE` accepts `all`, `chan`, or `chan-desktop` and rejects every other value before creating a guest.
- The driver preserves the combined build log and status, propagates failures, and removes its PID-scoped container on every exit path.
...
- The stubbed contract test rejects a missing explicit rootfs and a writable source bind.

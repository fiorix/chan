...
SHIPPED in v0.88.0, incompletely: the suite runs from a clean container through `make browser-smoke-deps`, and 23 network-idle waits across 19 checks now wait on the property each check consumes, including a window-registration barrier the rate wait had been supplying unnamed. Four of the five acceptance lines are met. The fifth is not: `56-external-edit-matrix` reached nine of ten consecutive runs on a loaded host, stopped by two defects this work uncovered rather than introduced, both registered separately at their own severity, an intermittent stale read on the editor's external-edit convergence path and an editor that intermittently never mounts. Explicitly NOT covered: the four network-idle sites in `gateway-zone-browser.mjs`, where the wait may be correct because those pages may hold no live transports; the suite's remaining fixed wall-clock bounds on genuine property waits; and the WebKitGTK blind spot that no Chrome-driven check can see, which running this suite does not close.

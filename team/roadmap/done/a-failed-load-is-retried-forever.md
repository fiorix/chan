...
Status: shipped in [v0.100.0](../../release/release-v0.100.0.md): A failed load shows where the content would have been and is not retried until something that could make it succeed changes.
...
- `components/FileInfoBody.svelte`: the effect that fetches a missing entry's parent directory guards on the entry alone. A parent that cannot be listed never produces the entry, so the effect re-arms its own fetch; against a stub bounded at five requests the inspector issued six, and unbounded it exhausted the test worker's heap.
...
`web/packages/workspace-app/src/components/PathPromptModal.svelte`, `components/SearchPanel.svelte`, `components/GraphPanel.svelte` (the latch is cleared by `reloadGraph`, the hide-reset and the scope-change path), `components/FileInfoBody.svelte`, and `state/store.svelte.ts` only if `dirErrors` needs a reader helper. Tests: `components/searchReadiness.test.ts` and new mounted tests for the other two. `GraphPanel.svelte` is read as source text by 31 suites.
...
4. Opening the inspector on an entry whose parent cannot be listed issues one request for that parent, asserted on a counting stub.

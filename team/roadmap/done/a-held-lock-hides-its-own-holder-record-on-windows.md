...
## The failure counts above do not reconcile, recorded 2026-08-14

The "10 failures before and 9 after" sentence cannot be squared with this item's own lists: the Grounding section pins five named tests failing before the change, the Shipped section adds a sixth, and fixing six of ten while breaking none leaves four, not nine. Either the before-count, the after-count, or the counting unit is misrecorded, and restating it needs the Windows box the measurement was taken on; the intake amendment has since changed the code a re-run would measure. The claim the totals were offered for, that the change breaks nothing else in the crate, rests on the named test lists, which are consistent with each other and with the tests that ship, not on the totals. Recorded in place rather than corrected by guess.

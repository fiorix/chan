...
Status: shipped in [v0.100.0](../../release/release-v0.100.0.md): The desktop recognizes a live-terminals refusal by its `live_terminals` discriminator and shows any other 409 with its own message.
...
`desktop/src-tauri/src/devserver.rs` (the four call sites and `SetWorkspaceOnError`), `desktop/src-tauri/src/main.rs` and `crates/chan-library/src/desktop_window_ops.rs` (the outcome the bridge carries, which has no variant for a refusal that is neither done nor a terminal count), and the launcher's rendering: `web/packages/launcher/src/state/computerActions.ts` (the confirm is on the off path only), `components/Library.svelte` (the `run` and `reportError` wrapper) and `api/library.ts` (`ApiError`, `refusalReason` and `liveTerminalsCount`, which already tells the two 409s apart and is the model to match). The desktop test module already binds a loopback axum router, so a 409 double needs no new harness. Sequence this item after [one-on-route-still-answers-204](one-on-route-still-answers-204.md), which reshapes the same bridge outcome. Making the server's locked refusal JSON as well was a possible companion change in `crates/chan-server/src/routes/library.rs`. Owner ruling, 2026-09-20: it stays plain text in this version and this item moves no server wire. The uniform answer is v0.101.0's, in [refusals-answer-in-four-shapes](../v0.101.0/refusals-answer-in-four-shapes.md), and this item prepares the reading side for it: the four call sites read a refusal through one function over the status and the body, which takes a JSON body's `error` as the discriminator when it is `live_terminals` with a count and as the message otherwise, and takes a body that is not JSON as the message. That is what the launcher's `refusalReason` and `liveTerminalsCount` already do together, and it means a server that later moves its plain-text refusals into the `{"error": ...}` envelope of `crates/chan-server/src/error.rs` needs no desktop change.
...
4. A JSON 409 whose `error` is a string other than `live_terminals` shows that string as the message, so the reader is ready for a server that answers its refusals in the `{"error": ...}` envelope.

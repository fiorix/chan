...
- `make shell-check`, the fdstore cleanup-order self-test, both Node syntax checks, `git diff --check`, and `cargo fmt` passed. A later aggregate six-file Vitest invocation did not complete and was interrupted, so only the individually completed focused runs above are reported green. The lead owns the final isolated pre-push gate.
- On 2026-08-04, `scripts/e2e/devserver-fdstore.sh` passed on host `dev1.fiorix.net` at commit `25c96fff`. The eight cases passed: (1) bare `systemctl --user restart`, including distinct live/spawn metadata; (2) `chan devserver --restart`; (3) watchdog restart after `SIGSTOP`; (4) `kill -9` crash restore with a second session; (5) session close removes its store entry; (6) `chan devserver --stop`; (7) `--restart --force`; and (8) bare `systemctl --user stop`. Fd-store checkpoints were 0 on fresh boot; 1 after the first spawn and after cases 1-3; 2 after the second spawn and crash adoption; 1 after case 5; 0 after case 6; and 1 before, then 0 after, cases 7 and 8.

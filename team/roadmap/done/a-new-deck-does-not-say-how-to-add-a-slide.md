...
The frontend seed mirror test `keeps the seed in slides mode without creating a page break` asserts that `parseSlidesSpec` still selects the slides layout, `PAGE_BREAK_RE` matches zero seed lines, `splitSlidePages` returns exactly one page containing the bullet, and appending a real `@pagebreak` line produces two pages. The test `lands at the end of the seed's # Slide 1 heading line` pins `firstSlideHeadingCaret` at the heading end. Present mode and deck PDF export both consume `splitSlidePages`, so the same one-page assertion covers their page count. The focused seed test passes, `slides.test.ts` passes 18/18, `page_break.test.ts` passes 9/9, and `cargo fmt --check` passes after the final Rust edit.

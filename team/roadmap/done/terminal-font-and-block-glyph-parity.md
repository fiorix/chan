...
Closed: shipped in [v0.88.0](../../release/release-v0.88.0.md), partial. The bundled face and the ghostty block glyphs shipped; the xterm DOM renderer the Linux desktop ships still bands rules and blocks, and that residual carries forward.

Status: REGISTERED 2026-08-10, after the fact. The work was done unplanned, off the roadmap, and is registered here so v0.88.0 carries it as accepted scope. IMPLEMENTED in `856c36e3` (the face), `eef8739a` (the block glyphs), `81f920a6` and `51fe458f` (the instrument that measures both, on Linux and on Windows). **Partial.** The bundled face, the `os-default` chain and the ghostty block glyphs shipped and are measured clean, as is everything the Windows desktop ships. The xterm DOM renderer the Linux desktop ships still bands rules and blocks at 96.0% rule continuity and 95.2% block coverage, **independently reproduced this round on a second machine sharing nothing with the first, gap positions included**, so the residual is a confirmed defect rather than one host's reading, and the scenario pack exits 1 on those arms by design. The WebGL present stall that keeps that renderer off remains **unmeasured**: no host with a GPU and an Xorg session was available in v0.88.0. It is no longer unmeasurable. `scripts/e2e/webgl-present-stall.py` settles it in one command, every branch of it has been exercised, and it sweeps the dma-buf variable that every earlier reading left uncontrolled. The re-check is a command, not a project. The question is carried forward as `v0.89.0/the-webgl-present-stall-is-unmeasured-and-costs-linux-the-grid.md` rather than left implied here.
...
**Acting on the answer is still unbounded if the stall reproduces.** That has not changed and should not be read as having changed by the measurement getting cheap. If it does not reproduce, turning WebGL on for the Linux desktop closes the residual outright and the whole thing is a one-line change plus whatever the browser smoke suite then observes differently.

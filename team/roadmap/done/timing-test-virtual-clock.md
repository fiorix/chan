...
## Follow-up: the bounded-reader drop tests assert per-file producers

A third instance of the class surfaced in CI days after this item closed: `fs_ops::tests::dropping_bounded_reader_joins_blocked_producer` and its slice sibling went red once on a loaded runner with `left: 1, right: 0` at the `BoundedFileReader::drop must close the queue and join its producer` assertion. The tests asserted a process-global count of active bounded-reader producers, and their serializing mutex excluded only each other: every other test in the binary opens bounded readers too, so a neighbor's live producer read as this test's un-reaped one. The producer guard now records `(id, root.join(rel))` and `active_bounded_file_readers(path)` filters to the caller's file, which no other fixture can collide with (each test has its own tempdir root). Discrimination proven both directions: a no-op-`drop` mutation leaves the producer blocked on the full queue and both tests red with the CI's exact `left: 1, right: 0` signature, and the full chan-workspace suite passes 15 of 15 runs under parallel vite builds.

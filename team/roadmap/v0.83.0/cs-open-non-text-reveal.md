...
- `open_path` (`crates/chan-server/src/control_socket.rs:3954`) is called by both the `cs open` control-socket handler and `routes/open.rs`. Its five branches are empty-path -> browser at root, dir -> `OpenBrowser` with `enter: true`, `is_editable_text` or `sniff_is_text` -> `OpenFile`, existing non-text -> the refusal at `:3999`, missing path -> create-then-open at `:4001`. Only the refusal branch changes.
...
- The classifiers it uses (`isImage` / `isPdf` / `isVideo`, `state/fileTypes.ts:286,297,306`) exist as this three-way list only in TypeScript. Rust classifies image and PDF but has no video class: `chan_workspace::fs_ops::classify` (`crates/chan-workspace/src/fs_ops.rs:350`) returns `FileClass::Image` and `FileClass::Pdf`, and `chan-server` already links it via `is_media_graph_path` (`routes/graph.rs:784`); `is_image_path` (`routes/graph.rs:541`) is a narrow graph-local helper. Neither covers video.

## Contract

- Server: `open_path`'s refusal branch stops returning `Err` and sends `OpenBrowser { path: parent_rel(&rel), select: Some(rel), enter: false, destination }`. `parent_rel` (`control_socket.rs:4067`) already exists for exactly this shape. The server decides "not text", nothing more.
...
Small. One branch in `open_path` plus its tests, one call in the SPA's `open_browser` handler, and the removal of the refusal string and the checks that pin it (`control_socket.rs:5219`, `routes/open.rs:288`, `scripts/e2e/browser-smoke/checks/80-launcher-open.mjs:81`). No new wire field, no new tab kind, no classifier work.

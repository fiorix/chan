# Hybrid Nav: staged draft and diagram editors get a tab bubble
...
`n` (new draft) has the same gap. The original note assumed drafts already showed a bubble; they do not. Both keys queue through the same function, so this item covers both.
...
The split exists for a reason. A draft editor needs a file on disk first: `materializeStagedDraftEditors` (`App.svelte:1012`) drains the queue only after commit, awaiting `api.createDraft()` or `api.createDiagram()` per entry before `openInPane`. The server picks the name (`crates/chan-server/src/routes/drafts.rs:159,196`, `next_untitled_draft_name` with a race retry at `:239,262`), so at press time neither the path nor the final tab title is known to the client.
...
- The chip carries a close affordance that drops that one queue entry (a small `paneModeUnstageDraftEditor`), and nothing else. Staged `t` / `o` / `g` / `b` tabs are ordinary draft tabs and can already be closed individually; without this, a stray `i` can only be undone by discarding the entire draft layout. There is no selection behavior, because there is no tab to select.

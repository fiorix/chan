...
| --- | --- | --- |
| [web-lock-check-destroys-node-modules](v0.86.0/web-lock-check-destroys-node-modules.md) | accepted, needs re-scope | the 2026-08-07 re-verification falsified the npm attribution: the installed npm 10.9.8 guards the delete phase under --dry-run; re-ground the incident, then close as environment-fixed with a pinned check or narrow to the unpinned comment claim |
...
| [large-transfer-ceiling-refinements](v0.86.0/large-transfer-ceiling-refinements.md) | accepted, specced; contract narrowed 2026-08-07 | bound archives cumulatively on both arms with refuse-before-first-byte semantics, settle Range charging against the workspace-arm read ruling; the recovery gap is overtaken by a source ruling and is confirm-and-strike |
...
| [source-pins-bound-on-sibling-string-literals](v0.86.0/source-pins-bound-on-sibling-string-literals.md) | accepted, needs a boundary ruling | fix the dead end-assertions and non-unique needles in the main.rs source pins; the 2026-08-07 re-verification found the dead-bound pattern at 24 sites, so the item must state whether it covers the three named pins or the class |
...
| [cs-terminal-new-cannot-spawn-an-agent-session](v0.86.0/cs-terminal-new-cannot-spawn-an-agent-session.md) | accepted, needs a route ruling | let cs terminal new set the spawn command and env so a single terminal can derive an agent and be poked; the 2026-08-07 re-verification found the server half already on HTTP and a restart-override path that covers the burn scenario, so the item must choose its surfaces |

...
| --- | --- | --- |
| [audit-the-workarounds-nobody-followed-up](v0.88.0/audit-the-workarounds-nobody-followed-up.md) | registered 2026-08-09, deferred at the v0.87.0 close | grep the workaround signatures and ask what else assumes the thing each one worked around; this is the mechanism that hid the mtime CAS data-loss path for three releases |
| [browser-smoke-is-unrunnable-and-rate-based](v0.88.0/browser-smoke-is-unrunnable-and-rate-based.md) | registered 2026-08-09, deferred at the v0.87.0 close | make the browser smokes runnable in the project's own containers and replace 19 networkidle2 waits with properties; the suite also has an engine blind spot Chrome cannot see |
| [chan-ps-cannot-answer-what-a-workspace-is-doing](v0.88.0/chan-ps-cannot-answer-what-a-workspace-is-doing.md) | registered 2026-08-09, deferred at the v0.87.0 close | surface the readiness, generation, required-action, and indexer columns `chan ps` already has access to |
| [control-socket-takeover-test-races-a-fixed-sleep](v0.88.0/control-socket-takeover-test-races-a-fixed-sleep.md) | registered 2026-08-09, deferred at the v0.87.0 close | remove the hardcoded 25ms sleep the takeover test races against its retry budget; the only inventory test with a mechanism named from source |
| [desktop-build-id-is-unknown-in-the-nix-package](v0.88.0/desktop-build-id-is-unknown-in-the-nix-package.md) | registered 2026-08-09, deferred at the v0.87.0 close | stamp a real build id into the Nix-built packages, `unknown` today because the flake source in the store has no `.git` |
| [doc-sessions-tests-stage-external-edits-on-the-filesystem-clock](v0.88.0/doc-sessions-tests-stage-external-edits-on-the-filesystem-clock.md) | registered 2026-08-09, deferred at the v0.87.0 close | remove doc_sessions tests' dependence on the filesystem clock advancing, including the hand-rolled 20ms sleep already papering over it |
| [one-stalled-workspace-may-block-the-others](v0.88.0/one-stalled-workspace-may-block-the-others.md) | registered 2026-08-09, unverified lead | reproduce first: establish whether a blocked `close_workspace` really exhausts tokio workers behind the global mount lock, or falsify it; no contract until the mechanism is established |
| [terminal-restart-env-test-is-load-sensitive](v0.88.0/terminal-restart-env-test-is-load-sensitive.md) | registered 2026-08-09, widened to a three-test cluster | name and fix the shared cause behind three `routes::terminal` tests failing under the same pressure |
| [the-boot-overlay-locks-the-workspace-behind-its-own-index-rebuild](v0.88.0/the-boot-overlay-locks-the-workspace-behind-its-own-index-rebuild.md) | registered 2026-08-09, deferred at the v0.87.0 close | stop locking the workspace for a recovery pass that is progressing normally, now that a stalled pass is distinguishable from a running one |
| [release-dry-run-does-not-predict-the-tagged-run](v0.88.0/release-dry-run-does-not-predict-the-tagged-run.md) | registered 2026-08-10 from the v0.87.0 GA failure | move the cached dmg venv out of `target/` and audit the other cached-tooling paths, so a publish=false dry run exercises the path the tagged run will; the v0.87.0 dry run passed the job the tag then failed, on the same commit |
| [devserver-restart-destroys-the-tunnel-registration](v0.88.0/devserver-restart-destroys-the-tunnel-registration.md) | unplanned, registered and implemented 2026-08-10 | closed a data-loss path where `--restart` rewrote the unit holding the only copy of the tunnel PAT; exercise it against a live supervised tunnel unit before the GA close |
| [terminal-font-and-block-glyph-parity](v0.88.0/terminal-font-and-block-glyph-parity.md) | unplanned, registered and implemented 2026-08-10, partial | the bundled face loads and ghostty blocks fill the cell, measured in the real desktop engine on Linux and Windows; the xterm DOM renderer Linux ships still bands rules and blocks, and whether the WebGL present stall that keeps it off still reproduces is the open question |
| [canvas-animations-are-software-rasterized-on-linux](v0.88.0/canvas-animations-are-software-rasterized-on-linux.md) | unplanned, registered and implemented 2026-08-10, partial | the whole family is on WebGL2; the point cloud host is correct by suite and unverified on a real GPU, and the vortex and bloom readings rest on a renderer string that does not distinguish hardware from software -- one run of `scripts/e2e/animation-fps.py` on a GPU settles all seven |
| [aur-publication-is-suspended](v0.88.0/aur-publication-is-suspended.md) | deferred, blocked upstream | restore AUR publication once the Arch incident notice is superseded and pushes are permitted; re-checked 2026-08-09 and still unmet |

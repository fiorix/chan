...
| [item-citations-anchor-on-content-and-are-checked](v0.89.0/item-citations-anchor-on-content-and-are-checked.md) | registered 2026-08-11, merged from two drafts that each declared the other blocking | a line number in a roadmap item keeps resolving after the code moves and nothing reports it, demonstrated on citations corrected during v0.88.0 that already resolve to the wrong lines one release later; forward-only, and it does not convert `done/` |

...
| [cs-open-non-text-reveal-and-audio](v0.84.0/cs-open-non-text-reveal-and-audio.md) | registered, grounded, specified, ready to implement | reveal existing non-text files through the File Browser and add browser-native preview for MP3, WAV, AIFF, and Ogg audio |
| [hybrid-nav-staged-editor-bubble](v0.84.0/hybrid-nav-staged-editor-bubble.md) | registered, grounded, specified, ready to implement | render removable draft/diagram chips and fail closed when a collaborator changes the staged layout domain |
| [terminal-tab-rename-reaches-inventory](v0.84.0/terminal-tab-rename-reaches-inventory.md) | registered, grounded, specified, ready to implement | settle live terminal name/group atomically on the server, preserve spawn provenance, and converge inventory and co-view clients |
| [terminal-editor-appearance-settings](v0.84.0/terminal-editor-appearance-settings.md) | registered, grounded, specified, ready to implement | add bounded terminal/editor font sizes and reversible custom terminal colours without changing standard rendering |
| [release-platform-verification](v0.84.0/release-platform-verification.md) | registered, grounded, specified, ready to implement | add a containerised mingw cross-check target and make it plus the `publish=false` dry run mandatory before a GA tag |

...
| [graph-from-here-on-a-directory-comes-up-without-its-files](v0.92.0/graph-from-here-on-a-directory-comes-up-without-its-files.md) | reproduced and fixed 2026-08-17, not gated or merged | the opening path and a re-scope render the same set, so that was not it; what reproduces is a directory whose immediate children are all directories, which opens at depth 1 with no file on screen while its own payload carries them, and a `dir:` scope now opens at the shallowest depth that holds a file |
| [the-linux-desktop-still-refuses-webgl-after-its-blocker-was-fixed](v0.92.0/the-linux-desktop-still-refuses-webgl-after-its-blocker-was-fixed.md) | accepted 2026-08-16, supersedes the v0.91.0 WebGL item | the stall does not reproduce and the dma-buf override became driver-scoped in v0.89.0, but `shouldUseWebglRenderer` still refuses every Linux desktop, so the grid ships at 96.0/95.2 against 100% elsewhere; make both decisions follow one driver signal, with a pixel measurement as the acceptance |
| [the-desktop-liveness-probe-test-is-load-sensitive-and-unexplained](v0.92.0/the-desktop-liveness-probe-test-is-load-sensitive-and-unexplained.md) | accepted 2026-08-16, carried out of a closed sweep item | measured 3 red in 15 full-binary runs on unmodified `main` and blocks a clean `cargo test --all-targets`; the probe reports live for a socket with no listener, so the timeout is the wrong lever, and a mechanism is wanted before any bound moves |
| [a-terminal-renderer-can-cache-glyphs-before-its-font-loads](v0.92.0/a-terminal-renderer-can-cache-glyphs-before-its-font-loads.md) | accepted 2026-08-17, implemented on `fix/terminal-font-readiness` | both terminal backends measure the cell and warm the glyph atlas from whatever font is resolved at construction, while the bundled Source Code Pro is a `font-display: swap` webfont leading every Linux chain, so a spawn during its load leaves one renderer holding fallback and webfont metrics at once; the branch awaits the face first and drops it from the chain when it cannot load |
| [the-about-widget-bottom-row-touches-the-window-edge](v0.92.0/the-about-widget-bottom-row-touches-the-window-edge.md) | accepted 2026-08-17, not started | the build id wraps the desktop About head to a second line, so the fixed 420x426 window clips the bottom margin the CSS already carries, and the Dashboard About slide has its own top-vs-bottom asymmetry; give both surfaces a bottom margin matching the top |
| [the-gateway-has-no-canonical-desktop-to-devserver-design](v0.92.0/the-gateway-has-no-canonical-desktop-to-devserver-design.md) | accepted 2026-08-17, not started | the gateway docs fragment and sometimes contradict how `chan-desktop` discovers an account, lists multiple owned or shared devservers, and enters one through its proxy; add one canonical gateway design with a placement diagram and publication, account/roster, and selected-devserver sequences, then remove stale and duplicate claims across the documentation set |

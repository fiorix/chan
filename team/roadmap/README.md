...
| --- | --- | --- |
| [an-external-edit-intermittently-never-reaches-a-dirty-editor](v0.92.0/an-external-edit-intermittently-never-reaches-a-dirty-editor.md) | resolved 2026-08-17 as not a defect; awaits GA close | the non-converging arm is a correct retained conflict after a partial flush advances the durable baseline, not a reconcile that never ran; `d8cdab63` characterizes the transition, while the preserved artifacts prove conflict but cannot establish the historical baseline or flush epoch |
...
| [the-about-widget-bottom-row-touches-the-window-edge](v0.92.0/the-about-widget-bottom-row-touches-the-window-edge.md) | implemented and browser-validated 2026-08-17; native observation outstanding | the desktop card now centers at 420x460 and the Dashboard slide carries a matching 2rem bottom pad; check 114 passed at height 460 with 32px above and below, but fixed-size WebKitGTK and WKWebView builds still need observation with the build id present and no scrollbar |

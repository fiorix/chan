...
| [freebsd-is-not-a-published-target](v0.96.0/freebsd-is-not-a-published-target.md) | port code merged; the release arm builds the static amd64 tarball; static intake found four defects in the FreeBSD-only code | settle the four findings, prove the arm in the round's `publish=false` dispatch, and run the FreeBSD install and upgrade acceptance on a real box |

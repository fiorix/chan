...
| [mtime-cas-silently-overwrites-external-edits](v0.87.0/mtime-cas-silently-overwrites-external-edits.md) | registered 2026-08-09 | give the mtime-only write CAS a content-hash backstop, so a flush cannot silently destroy an external edit written inside a non-advancing timestamp window; a data-loss path, not a test defect |

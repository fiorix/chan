...
| [release-artifacts-are-labelled-gnu-and-contain-musl](v0.89.0/release-artifacts-are-labelled-gnu-and-contain-musl.md) | registered 2026-08-10 by the owner during the v0.88.0 GA | the Linux CLI artifacts are named for gnu and contain musl tarballs; the same matrix field also requests a gnu toolchain target for a musl build, and both readings are wrong for what the job does |
...
| [canonicalize-failure-has-four-answers-on-the-path-sandbox](v0.89.0/canonicalize-failure-has-four-answers-on-the-path-sandbox.md) | registered 2026-08-11 from the v0.88.0 workaround audit, with the lexical-duplication draft folded in | a canonicalize failure is answered four different ways, two of them by falling back to a lexical check that cannot see symlinks; adopt the fail-closed ruling the crate already implements one layer up, and cover six branches no test reaches. No escape is claimed: the two fail-open arms feed an inspector badge, not a write gate |
| [chan-home-collapses-to-the-working-directory](v0.89.0/chan-home-collapses-to-the-working-directory.md) | registered 2026-08-11 from the v0.88.0 workaround audit; the owner ruled for a defined absolute fallback rather than refusal | an absent home resolves to a relative path, so a process whose home does not resolve writes the registry, the global config and every workspace's metadata into its own working directory and the user's workspace list simply reads as empty; add the absolute fallback and the injectable-home seam that makes the None arm reachable from a test at all |
| [chan-home-is-mutated-process-globally-during-a-parallel-suite](v0.89.0/chan-home-is-mutated-process-globally-during-a-parallel-suite.md) | registered 2026-08-11 from a v0.88.0 timing-lane draft the round kept out of scope | tests mutate `CHAN_HOME` process-globally while other threads read it, and separately `ensure_workspace_metadata_dirs` resolves through the ambient home even when the library was opened at an injected config path, so an isolated library still writes metadata to the real one; scoped to three named windows rather than to a project-wide ban on env mutation |
| [watch-registration-lifecycle-test-is-load-sensitive](v0.89.0/watch-registration-lifecycle-test-is-load-sensitive.md) | registered 2026-08-11 from a red observed during the v0.88.0 scoped gate | two health assertions race the retry they injected, over one process-global injection slot that four tests clobber, so a clobber does not only redden this test, it makes its neighbours pass vacuously |
| [indexer-timing-sites-have-no-lexical-signature](v0.89.0/indexer-timing-sites-have-no-lexical-signature.md) | registered 2026-08-11, carved out of a larger v0.88.0 draft that was not accepted | the shipped timing-site classification could only see sites carrying a `sleep`, so waits with no timing construct at all were structurally invisible to it; this reopens three sites that item classified as kept, which needs an explicit ruling before a lane starts |
| [the-sdme-build-drivers-are-uncapped-and-mount-a-live-worktree](v0.89.0/the-sdme-build-drivers-are-uncapped-and-mount-a-live-worktree.md) | registered 2026-08-11, merged from three v0.88.0 drafts that share one surface; the owner ruled on 2026-08-11 that btrfs is the only backend used here | no container-creating invocation names a storage backend, so all ten default to overlay, carry no btrfs qgroup entry and are invisible to the accounting every other container is measured by, and a disk cap cannot apply to them at all; separately three drivers bind the live worktree, so an edit during a long run yields a green verdict for a tree that never existed |
| [a-failed-reset-wedges-the-workspace-behind-a-retryable-error](v0.89.0/a-failed-reset-wedges-the-workspace-behind-a-retryable-error.md) | registered 2026-08-11, found during the v0.89.0 scope triage rather than carried from a draft | a reset that fails partway leaves the workspace cell empty with nothing in the tree to reinstall it, so the tenant answers `503` with `Retry-After` for the life of the process and a client retries a permanent failure forever, on a workspace the same call has already partly wiped; reproduced in-process on 2026-08-11 against the unrepaired control flow, and the item records both what was known at registration and what settled it |
| [settings-is-organised-by-concern-not-by-app](v0.89.0/settings-is-organised-by-concern-not-by-app.md) | registered 2026-08-11 by the owner, sized large at seven steps with two parallelisable | configuring the terminal means visiting two sections and the Graph and Dashboard have no section at all, because Settings groups by concern while the command registry has grouped by surface for longer than Settings has existed; derive the rail from the registry's own category set so the two cannot disagree about which surface owns a setting |
| [the-graph-palette-has-never-been-configurable](v0.89.0/the-graph-palette-has-never-been-configurable.md) | registered 2026-08-11 by the owner, after going looking for a setting that never existed | the graph's node colours have never been settable and a dead read-only legend implies they were; the `--g-*` tokens are the app's whole file-kind palette, so the override lands on the graph subtree rather than the root, and the filter menu is portaled to `document.body` so it needs a second application site or the dots and the canvas disagree |
| [the-deck-chords-are-invisible-to-the-shortcut-registry](v0.89.0/the-deck-chords-are-invisible-to-the-shortcut-registry.md) | registered 2026-08-11, from the owner asking for `Cmd+Enter` to present and finding the chord already taken | a slide deck's two chords are in none of the registry's 32 entries, so a rebind works everywhere except on a deck and nothing in the app advertises them; the owner ruled the shipped defaults stay and the two actions become rebindable instead |
| [ctrl-shift-w-closes-the-window-not-the-tab](v0.89.0/ctrl-shift-w-closes-the-window-not-the-tab.md) | registered 2026-08-11, from the owner using the Linux desktop app | off macOS the close chord a user reaches for discards the whole window where macOS closes the tab, and six layers can claim it; a small change whose deliverable is the verification matrix, with two open rulings on the control-terminal and launcher windows |
| [assigning-an-already-held-chord-has-no-swap-path](v0.89.0/assigning-an-already-held-chord-has-no-swap-path.md) | registered 2026-08-11 on the owner's ruling during the round, promoted from a draft the triage had not accepted | the assign dialog refuses a held chord and offers no next step, so the deck item makes the owner's own `Cmd+Enter presents` request expressible and then blocks it; promoted because shipping the deck item alone leaves that path worse than it is today, and promotion found that an unbound built-in is not a representable state |
| [the-1-cpu-reproduction-rig-has-no-checked-in-form](v0.89.0/the-1-cpu-reproduction-rig-has-no-checked-in-form.md) | registered 2026-08-11 on the owner's ruling during the round, promoted from a draft held rather than self-registered | the instrument that reproduces load-sensitive failures exists only as prose and as commands in a coordination tree deleted at each round close, so every timing item rebuilds it and none can say what the last one ran; costed by the lane that built it at about half a day and roughly 100 lines, against a cold cost every timing item pays again. Its load-bearing line is that the rig must fail closed when it cannot confirm the CPU cap from the host, since a silently uncapped rig reproduces the 400-green-runs-on-known-broken-code trap with a script attached |
| [agy-submit-agent](v0.89.0/agy-submit-agent.md) | registered 2026-08-12 on the owner's instruction, live-probed the same day | Google Antigravity's `agy` CLI, gemini's successor, is not a recognized submit agent: no derived identity, no `--submit=agy`, no chord; add it as a first-class agent with a live-probed bracketed-paste template, batching eligibility, and the full Rust and TypeScript mirror, while gemini stays supported with its deprecation planned for a later version, timing undecided |

The v0.88.0 round left seventeen drafts in the gitignored `dev/roadmap-drafts/` tree, and the 2026-08-11 triage decided all of them against the tree. Nine were promoted into the items above. Two were already decided before that triage ran: `the-webgl-present-stall-is-unmeasured-and-costs-linux-the-grid` is a stale pre-promotion copy of the accepted item listed above, and `the-point-cloud-animations-frame-rate-was-never-measured` was withdrawn at the v0.88.0 close once the animation family was confirmed on real Linux, Windows and macOS hardware. Six were deferred and still sit in that tree: `fd-probe-absence-is-assumed-impossible-on-unix`, whose boundary is falsified by a second copy of the probe in `chan-library`; the sweep half of `error-suppression-class-is-unaudited`, roughly 656 production sites against a first pass that took a round for 53; `wipe-dir-retry-budget-is-untested`, whose severe end is covered by the reset item above and whose own load-bearing claim is falsified by a test that did fail; `a-second-control-socket-takeover-test-races-the-same-budget`, whose named mechanism did not survive reading the code; `search-declines-outright-while-the-index-is-stale`, which names no contract and carries no acceptance checks yet; and the project-wide rule half of `tests-encode-implicit-scheduling-assumptions`, whose stated remedy assumes a test-harness timeout that does not exist. Those six exist only in that gitignored tree and are in no archive, so clearing it discards them. A draft is not an item until it is copied in and accepted.

The owner raised nine further drafts on 2026-08-11 covering the SPA, which are a separate population from the v0.88.0 seventeen. Four were promoted that afternoon and are the four SPA items above; a fifth, `assigning-an-already-held-chord-has-no-swap-path`, was promoted during the round once it was established that the deck-chord item would otherwise ship the owner's own request in a blocked state. The remaining four sit in the same gitignored tree and are not scope: `the-pill-palette-duplicates-the-concept-token-hexes` and `the-graph-background-and-text-colours-are-not-configurable`, each covering a token set disjoint from the graph palette's and each conflicting with it in the same two `App.svelte` blocks, so they cannot run beside it; `editor-chords-are-missing-from-the-shortcut-registry`, whose leading example did not survive verification, since no strikethrough keybinding exists anywhere in the tree; and `built-in-chord-supersession-is-checked-inconsistently`, which owns two live defects sitting inside the blast radius of the ctrl-shift-w item and is named out of scope by it deliberately.
...
Nothing reached this release without an item, but three of the thirteen were done unplanned and off the roadmap and registered after the fact so the release carries them as accepted scope rather than as unattributed lines: the canvas animation family, the `chan devserver --restart` tunnel-registration repair, and the terminal face and block glyph work. The release's other result has no item here at all, because it is a defect rather than a delivery: the browser smokes, runnable for the first time, found an intermittent stale read on the editor's external-edit convergence path on their first properly runnable execution, carried to v0.89.0 with a preserved reproducer rather than repaired here as [an-external-edit-intermittently-never-reaches-a-dirty-editor](v0.89.0/an-external-edit-intermittently-never-reaches-a-dirty-editor.md); the v0.87.0 mtime CAS is not implicated and nothing is silently lost. Of the fourteen items in scope, thirteen shipped and one deferred: [aur-publication-is-suspended](v0.89.0/aur-publication-is-suspended.md), still blocked upstream. The partial terminal item's residual carries forward as [the-webgl-present-stall-is-unmeasured-and-costs-linux-the-grid](v0.89.0/the-webgl-present-stall-is-unmeasured-and-costs-linux-the-grid.md), and what the canvas item exposed about writing a frame-rate acceptance that a software stack can satisfy carries forward as [a-frame-rate-acceptance-needs-guards-that-can-fire](v0.89.0/a-frame-rate-acceptance-needs-guards-that-can-fire.md). The seventeen drafts the round also produced were triaged on 2026-08-11 and their disposition is recorded in the Active section above.

...
**That duplication has already cost the project one round.** [settings-checked-checkbox-pill-border](../done/settings-checked-checkbox-pill-border.md), shipped in v0.85.0, had to remove one CSS declaration at three sites because the pill exists three times. All three still resolve and are now byte-identical in the block that item touched: `PillToggle.svelte:50-52`, `ReportsControl.svelte:127-129` and `SemanticControl.svelte:289-291` each read `.pill.on { background: var(--hover-bg); }`. The two copies have since drifted again in the other direction: both carry a `:has(input:disabled)` rule (`ReportsControl.svelte:130-133`, `SemanticControl.svelte:292-295`) that `PillToggle` does not have, because `PillToggle`'s props are only `checked`, `label` and `ontoggle` (`PillToggle.svelte:5-13`). So consolidating is not a delete: `PillToggle` has to grow a `disabled` prop first.
...
- Exactly one `.pill` block survives tree-wide and `PillToggle` carries the `disabled` prop its two copies already implement. Three exist today with identical `.pill.on` bodies (`PillToggle.svelte:50-52`, `ReportsControl.svelte:127-129`, `SemanticControl.svelte:289-291`), which is the same triple v0.85.0 had to edit three times.

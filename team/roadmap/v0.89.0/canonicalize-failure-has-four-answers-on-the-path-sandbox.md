...
Status: REGISTERED 2026-08-11 as v0.89.0 scope, carried forward from [audit-the-workarounds-nobody-followed-up](../done/audit-the-workarounds-nobody-followed-up.md), which shipped in v0.88.0 and recorded this as finding F2 without repairing it. The owner accepted it merged with a second draft from the same audit, on the lexical containment helper that exists twice; that draft is folded in below as a subordinate section rather than carried as its own item, because its whole surface sits inside this one's and because the ruling this item asks for may delete the code it wants consolidated. Verification against `f9c2878c` before promotion corrected the draft on three counts, each recorded in place below: the severity, the claim that no test constructs a missing root, and the scope of the word "every" in its contract.
...
The draft asked the maintainer to decide, from scratch, what a canonicalize failure means at the sandbox boundary. That decision has already been made and implemented in `chan-workspace` one layer above `fs_ops`, and the finding is better stated as four sites that do not conform to it.
...
The consequence worth stating plainly, because the draft's framing implies otherwise: **`root_canon` is never a fallback value.** It is either the result of a successful `canonicalize` or the call that needed it already failed. Nothing in the write path proceeds on a guessed root.

So fail-closed is the project's implemented position on this question, and what this item wants is the four `fs_ops` sites conforming to it. That is a decision to adopt a precedent, not a decision to invent one, and it removes most of the cost the draft attributed to the ruling.

## Severity, restated: this is a consistency defect and no escape is claimed

The draft's title said two of the four answers fail open and its rationale called this a security boundary. Both fail-open answers are reached only through `symlink_target_escapes_workspace`, whose entire output is one `bool` field, `PathClass.target_escapes_workspace` (declared fs_ops.rs:184, computed fs_ops.rs:239-241). That field is reporting data on the wire. `classify_path` is called for it by `path_class_for_wire` (routes/files.rs:302), by the inspector payload (routes/inspector.rs:107), by `path_class_for_graph` (routes/graph.rs:884) and by the fs-graph walker (routes/fs_graph.rs:1128 and :1299), and the SPA renders it as a single chip reading "outside workspace" (`web/packages/workspace-app/src/components/FileInfoBody.svelte:248`). **No write is gated on it anywhere.**
...
The draft says "No test in the crate constructs an uncanonicalizable root or an uncanonicalizable parent." That is false as written, and the correction matters, because the difference between an untested case and an unreachable one is the difference between two different items.
...
So this is a maintainability hazard and not a live containment bug. Both functions take two `&Path` arguments, so a transposition compiles. The draft's description of what a transposition would do is imprecise and is corrected here: passing `(path, root)` into the `(root, path)` copy computes `root.strip_prefix(path)`, which for an ordinary outside target such as `/etc/hosts` returns `Err` and refuses, which is fail closed. The genuine fail-open case is narrower and real: a target that is a strict ancestor of the root strips cleanly into all-`Normal` components, so a symlink pointing at the workspace root's parent would be reported as contained.

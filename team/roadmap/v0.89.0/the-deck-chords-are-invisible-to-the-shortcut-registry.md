...
2. The candidate goes to `keymapConflicts` (keymapAssign.ts:31) against `resolvedKeymapEntriesForSlot` (keymapOverrides.svelte.ts:120), which is built from the 32 `SHORTCUTS` entries plus catalog commands that already carry an override. Not one of the 32 chords contains `Enter`. So the conflict list is empty. (Corrected 2026-08-11: this sentence continued "the only non-modifier keys in the whole registry are `Backspace`, `Esc` and the digit range", which is false - the registry also carries letters and punctuation, as `shortcuts.ts:21` states in its own words. The argument needs only the narrower claim, that no registry chord's key is `Enter`, and that one holds.)

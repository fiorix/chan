...
A user who assigns `Cmd+Enter` to another command is silently shadowed by the deck; the reverse direction, shadowing the deck, is not reachable, and stating which direction is live matters because the two have different victims.

The rebinding path, end to end:
...
2. The candidate goes to `keymapConflicts` (keymapAssign.ts:31) against `resolvedKeymapEntriesForSlot` (keymapOverrides.svelte.ts:120), which is built from the 32 `SHORTCUTS` entries plus catalog commands that already carry an override. Not one of the 32 chords contains `Enter`. So the conflict list is empty. (The registry's non-modifier keys include letters and punctuation as well as `Backspace`, `Esc` and the digit range, as `shortcuts.ts:21` states in its own words; the argument needs only the narrower claim, that no registry chord's key is `Enter`, and that one holds.)

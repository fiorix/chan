...
The draft left the question open (refusal or defined absolute fallback), and the size of the item swung entirely on it. The ruling closes it: **a defined absolute fallback**. `config_dir()` keeps its `-> PathBuf` signature; the `home_dir() == None` arm resolves to a named absolute path instead of `.chan`, and that path is written into the docstring so the next reader does not have to derive it.
...
Small, and the ruling is what makes it small: one file, one new function, one changed expression, and two strengthened assertions. The refusal branch would have been 22 external call sites across chan-workspace, chan-server, chan and `desktop/src-tauri` plus 4 in-file delegators, which is where the draft's flat "small for the fix" was wrong, since it priced only the branch that had not been chosen.

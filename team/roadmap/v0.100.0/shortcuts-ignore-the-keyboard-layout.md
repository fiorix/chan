...
Status: accepted for v0.100.0 by the owner, carried from a plan agreed before v0.99.0 and not implemented in it (neither reviewed commit is in the repository, and nothing under `web/` or `desktop/` matches letters by layout). **It is sequenced behind the frontend review**, which the owner is revisiting before deciding what to do with it: complete or settle that work first, then fold this import into the resulting frontend structure. This document records the agreed plan; it does not authorize a push or release.

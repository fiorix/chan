...
Status: accepted for v0.100.0 by the owner on 2026-09-20, after reading the v0.94.0 history below, and sequenced at the end of the Rust lane's queue just before the `dump-skill` item: it moves to v0.101.0 with that item if the queue runs late. Raised the same day on the owner's instruction, during the round: "`chan open` as an alias for `chan serve`, and also add `chan close --forget` as an alias for `chan workspace forget`". Both spellings reverse part of a ruling the owner made in v0.94.0, recorded below so the reversal is deliberate. Behaviour was checked with the installed `chan 0.99.0 (build git-85a3fb1e4364)`; the code was read at `main` `d7b91cbbe`.

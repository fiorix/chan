...
Status: accepted for v0.100.0 by the owner on 2026-09-20, raised during the round by the shell lane while it fixed [a-tab-reorder-drops-live-tab-state](a-tab-reorder-drops-live-tab-state.md). Sequenced after that lane's second round, and it moves to v0.101.0 if that queue runs late instead of holding the release. The payload and the rebuild below were re-read against `main` at `ad8ef6087`; the list of lost fields is the lane's reading of the tab type.

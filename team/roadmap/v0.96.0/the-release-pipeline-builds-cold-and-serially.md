...
- The build-script fix was proven both ways in the integration container: with the old script a second `cargo build -p chan-server` recompiles `chan-server`; with the new one it compiles nothing (`Finished` in 0.72 s).

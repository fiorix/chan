...
- [release-v0.95.0](release-v0.95.0.md) - the remote workspace arms (`--on TARGET`) over the desktop handoff, self-upgrade for the Linux AppImage and the Windows install, the cs prefix grammar for the chan tree, and a structured close review over every commit since v0.94.0 that landed its findings as fixes before the cut; the most expensive mistakes were operational (a dropped `cd`, a gate verdict read from the wrong exit code), not product defects.
- [release-v0.95.1](release-v0.95.1.md) - a one-fix patch from using v0.95.0: the desktop's gateway arm turns a workspace off again by speaking the launcher's `/off` route instead of re-posting `/on`, pinned by a pure route test and a recording stand-in round trip; the defect sat in exactly the arm the v0.95.0 report had named unproven.
- [release-v0.96.0](release-v0.96.0.md) - a three-item integration round: FreeBSD becomes a published target on amd64 and arm64, the release pipeline builds warm and in parallel instead of cold and serially, and a terminal recovers keyboard focus after macOS wake; intake found four defects in FreeBSD-only code that four green dispatches could not reach, the owner ruled them fixed rather than deferred, and the sharpest of them, a traversal whose descriptors scaled with tree width, was caught by a reviewer after the code had passed three compile checks; arm64 was scoped out and added back at the close, after the GA commit had already been staged and proven, because the owner declined to split one platform across two releases.
- [release-v0.97.0](release-v0.97.0.md) - a direct-GA round: the standalone Windows x64 CLI gains a SHA-verified PowerShell installer and real self-upgrade while preserving the desktop's NSIS route, stock FreeBSD measures descriptor pressure and defaults devserver management to chan's daemon, and reindex pacing becomes bounded; native Windows CI proves the install, refusal and replacement paths, while the workstation tooling mistake that preceded it is recorded as the process defect it was.

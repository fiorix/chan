...
- **PLACEHOLDER (Lead completes at GA): two live samples of one lock disagree.** Done: the Rust work from the owner's agent (`956758104`, `b9530551d`, `a3b92b9d4`) makes the lock probe three-state (absent, present, and unknown with a reason), maps unknown to a new `unknown` workspace status on the wire, makes `chan workspace search` and `chan workspace status` refuse on unknown, and has the equality test compare one recorded snapshot. The launcher reads `unknown` as its own condition with every action withheld (`1725289bf`). A `ulimit -n` sweep reproduced the original mismatch once in 100 baseline runs and never on the candidate. Remaining: the gate on `17689e826` and the landing.
...
- **PLACEHOLDER (Lead completes at GA): letter shortcuts follow the keyboard layout.** Done: Laurie Clark-Michalek's layout-aware letter matching (two commits, authorship preserved), plus logical punctuation and Shift handling across the workspace, the launcher, both desktop bridges, terminal escape, capture and overrides, and extension keyboard relay v2. The branch `v0100/shell-layout` (`c6763f3cd`) is green on its scoped Chan checks, and the echo extension round trip passes on the candidate and on a main baseline. mobile-chat (`b8f0001e5`) and Doom (`053121201`) are fast-forwards of their mains with their gates green. Doom's two `doom-server` reds turned out to be a fixture race, fixed in Doom's own repository. Remaining: the rebase onto main after lock-samples, the full gate, and the three coordinated landings. The macOS Colemak, Dvorak and Option checks are pending for the contributor after release.
...
Every gate read 0 failed and 6 ignored. All four Svelte checks were clean, the host and AppImage devserver smokes passed, and the anchored failure scan was empty. The gate-log audit tool used on each was first shown to go red on four doctored logs. <!-- GA: add the lock-samples, keyboard-layout and final-head gates. -->

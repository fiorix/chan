...
Rules:
  ...
  - `skipped_entries` is optional on the `meta` record: the number of entries the scan skipped, present only when non-zero, so a record without it describes a scan that skipped nothing. `load_jsonl` restores it into `Index::skipped_entries()`.
...
The walker emits relative POSIX paths to the counter. Unrecognized languages are dropped silently. Non-UTF-8 content is decoded in memory with tokei's decoder options (section 7); it is not necessarily binary or uncountable. Unreadable descendants, paths outside the root, non-UTF-8 file names, and per-file counting errors are skipped so siblings remain reportable. Errors traversing the root itself, I/O errors without path context, and invalid exclude overrides still fail the scan. A pathless I/O error can originate from the root's directory listing, so it cannot safely be classified as a failed descendant. `Index::skipped_entries()` exposes the number of entries skipped because walking or counting failed during the scan that produced the index. The count travels in the JSONL `meta` record (section 4), so an index loaded from JSONL reports the count of the scan that wrote it, and every snapshot's `meta` carries the index's count whatever its scope, because the index keeps no per-path record of skips, so no scope can tell whether a skip fell inside it. chan-workspace logs one warning with the count when opening or replacing a report whose index skipped entries.

chan-workspace persists an index whether or not its scan skipped entries: every flush, including the eager flush after an open or a policy replacement, writes the JSONL with the count in its `meta` record. `ReportState::open` loads that cache like any other and logs the warning with the stored count, so the first `report()` and `report_if_available` serve the partial index at once. A workspace opened through `Library` still has every persisted report refreshed by a full scan on its startup recovery worker, as for a complete cache (the persisted-report rescan in `crates/chan-workspace/design.md`), so an entry that stays unreadable costs one background scan per open, with the partial index served meanwhile. Later per-file updates do not change the count; only a replacement scan produces a new one.
...
Unit seams and integration tests with tempdir-built mixed-language trees pin the behavioral contracts:
  ...
  - Scan errors: injected descendant traversal, non-UTF-8 path, out-of-root path and per-file counting failures preserve good siblings and count skips; root traversal and pathless I/O errors remain fatal. JSONL round trips carry the skipped-entry count, a `meta` record without it loads as zero, a complete index writes no such field, and every scope's snapshot reports the index's count.
  - Workspace persistence: a scan with an injected counting failure is flushed to JSONL with its count in `meta`; a subsequent `ReportState::open` over that cache loads it without scanning, and its eager flush rewrites the file with the stored count.
  - Counter: language detection, SLOC, and complexity vs. known fixtures written inline.
  - Read cap: recognized oversized files retain metadata without a content read; deterministic growth after stat uses a bounded read, the open handle's size/mtime and zero line counts. Small shebang and non-UTF-8 files remain countable; oversized shebang-only files are untracked. Extensionless non-shebang files and overlong shebang lines do not enter the body reader. Parity tests check path-only detection against tokei for every mirrored filename, dotted named files and representative extensions; the bounded shebang detector against tokei's own detector for every `#!` path and `env` interpreter it knows plus edge cases, including a path or interpreter name extended by a suffix, with a first line ended by EOF or by a bare `\r` pinned as first lines the probe rejects and tokei detects (a first line longer than the probe is the other such case); and non-UTF-8 counting against tokei's path parser for UTF-16 with either byte order, Latin-1 and a UTF-8 BOM over invalid bytes.

...
- **Terminal tenant owner gate**: the slim workspace-less terminal router serves a PTY plus a uid-scoped transfer lane, so the gateway's read-only guest shape has nothing to mean on it. `auth_middleware` waves anything carrying `TunnelOrigin` past the tenant bearer (that bypass is what implements the workspace tenant's guest mode) and the gateway assertion carries no path or workspace scope, so without a gate of its own a non-owner session reaches `POST /api/terminals` and an unrooted read/write over the whole uid. A route layer over the tenant's whole `/api` surface answers a non-owner gateway role 403, the same refusal shape `require_tunnel_owner` gives the reverse-tunnel legs. The SPA shell served by the fallback stays public like every other static asset, and a caller carrying no `TunnelOrigin` (the desktop, `cs`) keeps the bearer lane it had.

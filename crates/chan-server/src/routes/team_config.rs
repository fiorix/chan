...
/// Generate a paste-and-run bootstrap shell script for `config` at the
/// workspace-relative `team_dir`. This is the `--script` form of the team
/// bootstrap: run from a chan terminal it recreates the WHOLE team using
/// only the public `cs` surface plus plain shell:
///
...
/// result, and uses that one value for the tree it writes and for the pokes
/// it sends, so the two cannot disagree wherever it runs. `team_dir` is the
/// form the CLI resolved at generation time, so the tree lands where the
/// direct spawn puts it only when the script runs from the workspace root;
/// run anywhere else, the members are still pointed at where it is.
///
...
pub(crate) fn generate_bootstrap_script(
    ...
    out.push_str(
        "# Generated by `cs terminal team ... --script`. Paste-and-run from a chan\n\
         # terminal (one with $CHAN_CONTROL_SOCKET + $CHAN_WINDOW_ID set). The team\n\
         # directory is resolved against $PWD once, below; the tree is written\n\
         # there and every identity poke names that same path.\n",
    );
    out.push_str("set -euo pipefail\n\n");

    // One resolution of the team directory, shared by the tree and the pokes,
    // so the two cannot disagree wherever the script runs. The variables carry
    // a chan_ prefix because a pasted script runs in the caller's own shell.
    out.push_str("# 1. Team directory tree, under $PWD.\n");
    out.push_str("chan_team_root=$PWD\n");
    out.push_str(&format!(
        "chan_team_dir=\"$chan_team_root\"/{}\n",
        sh_squote(dir)
    ));
    out.push_str("printf 'team directory: %s\\n' \"$chan_team_dir\"\n");
    out.push_str(
        "mkdir -p \"$chan_team_dir\"/tasks \"$chan_team_dir\"/journals \"$chan_team_dir\"/followups\n\n",
    );

    out.push_str("# 2. config.toml (the validated team config).\n");
    let config_eof = heredoc_delimiter("CHAN_TEAM_CONFIG_EOF", &config_toml);
    out.push_str(&format!(
        "cat <<'{config_eof}' > \"$chan_team_dir\"/config.toml\n"
    ));
    ...
    out.push_str(&format!(
        "cat <<'{bootstrap_eof}' > \"$chan_team_dir\"/bootstrap.md\n"
    ));
    ...
    for &m in &order {
        ...
        let prompt = IdentityPrompt::for_member(config, m);
        // --submit makes the poke fire instead of parking in the compose
        // box; the server derives the target's chord at enqueue, and the
        // <agent> named here is the same derived roster value. The two paths
        // the poke names are spliced in as double-quoted expansions between
        // the $'...' pieces, where `ansi_c_escape` keeps `$` literal.
        out.push_str(&format!(
            "cs terminal write --tab-name={} --submit={agent} $'{}'\"$chan_team_dir\"$'/bootstrap.md{}'\"$chan_team_root\"$'{}'\n",
            sh_squote(&m.handle),
            ansi_c_escape(&prompt.before_bootstrap),
            ansi_c_escape(&prompt.before_root),
            ansi_c_escape(&prompt.after_root),
        ));
    }
...
/// The per-agent identity poke, split around the two absolute paths it
/// names: the generated `bootstrap.md` to read, and the directory that
/// document's relative paths resolve against. `bootstrap.md` keeps its
/// workspace-relative paths because it is a persisted, shareable workspace
/// file (`team load` respawns a saved team without regenerating it), so the
/// poke is what anchors them; a team directory is commonly gitignored, and an
/// agent that searches for the file instead finds nothing.
///
/// The server-side spawner has the workspace root in hand and renders the
/// pieces through [`identity_prompt`]; the `--script` form knows the paths
/// only at run time, so it emits the pieces as bash `$'...'` strings and
/// splices the `$PWD`-derived values between them. Personalized per member
/// (the CLI pokes each tab individually, unlike the SPA's single lead-editor
/// prompt). ASCII only.
pub(crate) struct IdentityPrompt {
    /// The text up to the `bootstrap.md` path.
    pub(crate) before_bootstrap: String,
    /// The text between the `bootstrap.md` path and the anchor directory.
    pub(crate) before_root: String,
    /// The text after the anchor directory.
    pub(crate) after_root: String,
}

impl IdentityPrompt {
    /// The pieces for `member`: who it is, and what to do once it has read
    /// the process (the lead distributes, a worker waits to be assigned).
    pub(crate) fn for_member(config: &TeamConfig, member: &Member) -> Self {
        ...
        let next_step = if member.is_lead {
            format!(
                "distribute the work and coordinate with {}",
                config.host_handle
            )
        } else {
            format!("wait for {lead_handle} to assign your task")
        };
        Self {
            before_bootstrap,
            before_root: format!(
                ", identify yourself, then {next_step}.\n\
                 Relative paths in that document resolve against "
            ),
            after_root: ".".to_string(),
        }
    }

    /// The poke with the two paths filled in.
    pub(crate) fn render(&self, bootstrap: &str, root: &str) -> String {
        format!(
            "{}{bootstrap}{}{root}{}",
            self.before_bootstrap, self.before_root, self.after_root
        )
    }
}

/// The identity poke for `member` of the team at the workspace-relative
/// `team_dir` under `workspace_root`, naming the absolute `bootstrap.md` and
/// the root itself. The root is shown without a Windows verbatim prefix, the
/// spelling the registry keeps for registered roots, so the poke reads the
/// same however the root arrived. The server-side spawner appends the
/// agent's submit chord to this via `apply_submit_chord`, the same bytes the
/// script's `--submit` produces.
pub(crate) fn identity_prompt(
    config: &TeamConfig,
    workspace_root: &Path,
    team_dir: &str,
    member: &Member,
    ...
    // `team_dir` is a wire path with `/` separators; joining it component by
    // component gives the platform's own spelling.
    let bootstrap = team_dir
        .split('/')
        .filter(|component| !component.is_empty())
        .fold(root.clone(), |path, component| path.join(component))
        .join("bootstrap.md");
    IdentityPrompt::for_member(config, member)
        .render(&bootstrap.to_string_lossy(), &root.to_string_lossy())
}
...
mod tests {
    ...
    fn bootstrap_derives_kimi_and_documents_its_submit_encoding() {
        ...
        let bootstrap = generate_bootstrap_md("new-team-1", &config, None);
        assert_eq!(member_agent(&config.members[1]), Some("kimi"));
        assert!(bootstrap.contains("kimi"), "kimi member in roster");
        assert!(
    ...
    fn script_writes_tree_config_bootstrap_and_spawns_lead_first() {
        ...
        // The team directory resolves once against $PWD, is printed, and is
        // the one path the tree is written under.
        assert!(
            script
                .contains("chan_team_root=$PWD\nchan_team_dir=\"$chan_team_root\"/'new-team-1'\n"),
            "one resolution of the team directory: {script}"
        );
        assert!(
            script.contains("printf 'team directory: %s\\n' \"$chan_team_dir\"\n"),
            "prints the resolved directory: {script}"
        );
        assert!(
            script.contains(
                "mkdir -p \"$chan_team_dir\"/tasks \"$chan_team_dir\"/journals \"$chan_team_dir\"/followups"
        ...
        assert!(
            script.contains("> \"$chan_team_dir\"/config.toml"),
            "config write"
        );
        assert!(
            script.contains("> \"$chan_team_dir\"/bootstrap.md"),
            "bootstrap write"
        );
        ...
        // The poke splices the shell-resolved paths between $'...' pieces,
        // outside the ANSI-C quoting that keeps `$` literal.
        assert!(
            script.contains(
                r#"cs terminal write --tab-name='@@Lead' --submit=claude $'# Team work\nYou are @@Lead on team "alpha" (a team of 2; host @@Neo, lead @@Lead).\nRead the team process at '"$chan_team_dir"$'/bootstrap.md, identify yourself, then distribute the work and coordinate with @@Neo.\nRelative paths in that document resolve against '"$chan_team_root"$'.'"#
            ),
            "lead poke splices the resolved paths: {script}"
        );
        assert!(
            script.contains(
...
sleep 3"
    ...
    fn identity_prompt_addresses_the_member_and_points_at_bootstrap() {
        ...
        let bootstrap = root.join("new-team-1").join("bootstrap.md");
        let lead_prompt = identity_prompt(&config, root, "new-team-1", &config.members[0]);
        assert!(lead_prompt.contains("You are @@Lead"), "names the member");
        assert!(
            lead_prompt.contains(&format!(
                "Read the team process at {},",
                bootstrap.display()
            )),
            "points at the absolute bootstrap.md: {lead_prompt}"
        );
        assert!(
            lead_prompt.ends_with(&format!(
                "Relative paths in that document resolve against {}.",
                root.display()
            )),
            "names the root the document's paths resolve against: {lead_prompt}"
        );
        assert!(
            lead_prompt.contains("distribute the work"),
            "lead-specific guidance"
        );
        let worker_prompt = identity_prompt(&config, root, "new-team-1", &config.members[1]);
        ...
        assert!(
            worker_prompt.contains(&format!("at {},", bootstrap.display())),
            "the worker is pointed at the same file: {worker_prompt}"
        );
    }

    #[test]
    fn identity_prompt_shows_a_verbatim_root_without_its_prefix() {
        // The registry strips the Windows verbatim prefix from registered
        // roots; the poke renders that spelling however the root arrives.
        let config = sample_config();
        ...
        assert!(!prompt.contains(r"\\?\"), "no verbatim prefix: {prompt}");
        assert!(prompt.contains(r"C:\ws"), "drive path kept: {prompt}");
    }

    #[test]
    fn identity_prompt_joins_the_team_dir_by_component() {
        // A nested directory with a trailing slash joins as path components,
        // so the poke never carries a doubled separator.
        let config = sample_config();
        let root = Path::new("/ws");
        let prompt = identity_prompt(&config, root, "teams/alpha/", &config.members[0]);
        let expected = root.join("teams").join("alpha").join("bootstrap.md");
        assert!(
            prompt.contains(&format!("at {},", expected.display())),
            "{prompt}"
        );
    }

...
12. `run_tunnel(...)` runs until close or eviction. On exit, `registry.deregister_if_owner(&handle)`, and the handler's return drops the sender that tells the h2 driver (step 6) to close the connection.

...
8. Verify the validated token's `scopes` contains `"tunnel"`; otherwise return the same empty 401 as `InvalidToken` while retaining `ServerError::MissingScope` for logs.

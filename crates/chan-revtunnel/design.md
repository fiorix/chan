...
- The unguessable 32-char `tunnel_id` is the only capability naming a tunnel on the two public paths; over the gateway the legs additionally pass the gateway-session cookie, the exact Origin, and the owner's-desktop gate (a grantee's valid session is not authority to open sockets on the owner's machine, and neither is the owner's browser session: the assertion must name the owner on a session the gateway minted for the desktop app).

//! Registration locks keyed by a workspace's canonical root.
//!
...
//! confines that wait to callers of the same root.
...
/// One asynchronous mutex per workspace root, so the lifecycle calls of one
/// root serialize with each other and never with another root's.
///
...
///
/// Lock order: a caller holds at most one root lock, and takes it before any
/// other lock on its path: the routing map, the mount-state map, the
/// overlay's and the window registry's data and save locks, and the
/// library's registry mutex. Those are taken and released while the root
/// lock is held and none is held while a root lock is awaited. The entry
/// map's own mutex is a leaf: held only to look up, insert or prune an entry,
/// never across an await and never while another lock is taken or released.
///
/// An entry is a `Weak` to its root's mutex, and every caller waiting on or
...
/// callers different mutexes for one root. The last guard of a root removes
/// its entry, and every lookup prunes the entries a cancelled waiter left
/// behind, so the map holds only roots with a caller in flight.
#[derive(Default)]
pub(crate) struct RootLocks {
    entries: Mutex<HashMap<PathBuf, Weak<AsyncMutex<()>>>>,
}

impl RootLocks {
    /// Wait for the lock of the root whose canonical key is `key`.
    pub(crate) async fn lock(&self, key: &Path) -> RootLockGuard<'_> {
    ...
    fn mutex_for(&self, key: &Path) -> Arc<AsyncMutex<()>> {
        ...
        entries.insert(key.to_path_buf(), Arc::downgrade(&mutex));
        mutex
    }
...
/// One root's lock, held until drop.
pub(crate) struct RootLockGuard<'a> {
    locks: &'a RootLocks,
    key: PathBuf,
...
impl Drop for RootLockGuard<'_> {
    fn drop(&mut self) {
        ...
        // caller that looks the root up in between either shares this mutex,

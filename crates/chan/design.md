...
`chan open PATH` is a visible alias of `chan serve PATH`, using the same parsed arguments and URL refusal. `cs open` opens a file or directory in a window. `chan close --forget PATH` dispatches to the same teardown and registry removal as `chan workspace forget PATH`, including `--on` and the live-terminal refusal. `chan workspace open` is not a spelling.
  ...
  - `ps`, `config`, `completions`, `dump-skill`: the served-workspace listing, persisted preferences, shell-completion generation, and the agent-facing manual rendered from chan's own help text. `dump-skill` installs an index by default; `--list` omits its frontmatter, `--topic SLUG` reads a page or its part index, and `--topic SLUG --part N` reads one numbered part. Each output fits in 8 KiB except the explicit `--full` export. Parts concatenate byte-for-byte to the complete page. `cs dump-skill` uses the same selectors and renderer without a running server.
...
flowchart TB
  ...
  Reg -->|"ReplyLost: mount may still be running, or the devserver died"| Refuse
...
Every successful handoff returns early and never double-opens. A sent registration whose reply times out, never arrives or is invalid refuses a competing standalone open because the devserver may still finish mounting. Other failures (no desktop, refused, stale socket, version skew, GUI absent) drop through to the standalone serve; an explicitly selected devserver that is no longer live is also refused. `chan devserver` reuses the same `chan-server` machinery through `run_devserver` for the multi-workspace case.

...
`Personality` exists because the same CLI code runs from two binaries that must behave differently in exactly two places, and threading one enum is cheaper and clearer than two code paths or a build flag. `Standalone` is the `chan` binary from `install.sh` (and the `cs -> chan` symlink); `Desktop` is `chan-desktop` dispatching `chan` in-process through its shim. The forks:
  ...
  - **`chan upgrade`** -- `Standalone` replaces the CLI tarball in place (on Windows, where no CLI tarball is published, the NSIS install's console `chan.exe` carries `CHAN_DESKTOP_HANDOFF=1` and drives the desktop updater instead, while the standalone zip refuses); `Desktop` drives the desktop's `tauri-plugin-updater`, since a desktop install is not a loose binary it can overwrite.
...
The self-upgrade path keeps the running CLI current without a package manager. It has three pieces, all pointed at hardcoded `chan.app` metadata URLs (self-hosted mirrors are not supported for the CLI path):
  ...
  - **`chan upgrade`** (`Standalone`) -- resolves the running binary via `current_exe`, reads metadata, downloads the target archive into a sibling temp file, verifies its SHA-256 against the metadata, extracts the `chan` binary, and atomically renames it over the running executable. Size-capped and proxy-aware via the standard `*_PROXY` env vars. On Windows no CLI tarball is published: the NSIS install's console `chan.exe` (marked `CHAN_DESKTOP_HANDOFF=1`) hands the upgrade to the desktop and the standalone zip refuses. A distro-packaged build refuses `chan upgrade` and defers to the package manager.

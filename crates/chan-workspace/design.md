...
`Workspace::copy` connects `BoundedFileReader` directly to the bytes-mode atomic sink, which writes a file copy under the destination's own name inside a private stage directory beside the destination; the staged file is then renamed onto the destination with a rename that refuses an existing one (see `no_replace`), so a file or directory created after the absence check is kept and the copy is `PathAlreadyExists`. A new copy therefore inherits the byte budget, incremental UTF-8 validation when the destination is editable text (the staged path classifies as the destination does), a commit that never crosses filesystems, and stage cleanup on source, sink, or publish failure. The source's open-handle size is checked against the sink budget before the first chunk is accepted.

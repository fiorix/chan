...
## 8. Schema versioning
  ...
  - **Graph DB**: `PRAGMA user_version`. Migrations are idempotent and applied on every `GraphView::open`. Current version: 7 (v2 added `basename`, v3 added `emails`, v4 added staging tables for resumable reindex, v5 added a `size` column on `nodes` + `staging_nodes` so `Workspace::reconcile` can detect same-mtime-different-content rewrites that a mtime-only diff would miss, v6 added `aliases` so the mention resolver can map `@@<alias>` to a contact without re-parsing frontmatter per query, v7 adds the separate `text_files` stat table). Each `v < N` block wraps its schema change and the `PRAGMA user_version = N` bump in a single `unchecked_transaction` so a crash mid-migration leaves the DB at the previous version with intact data. A `column_exists` helper guards the `ALTER TABLE ADD COLUMN` calls so a crash before commit, which leaves the column physically added but `user_version` at the prior value, is safely re-run on the next open without `duplicate column name` errors.

The graph rebuild collects readable text-file stamps from its walk and replaces `text_files` in the same transaction as the Markdown staging swap. Text stamps do not create document nodes. An older binary skips migration blocks for unknown higher `user_version` values and ignores the additive table; a newer open reconciles changed, missing, and index-only paths again. A text stamp is not proof of an index entry: if the graph rebuild stamped a file whose subsequent index read failed, or an older binary forgot its entry without updating the stamp, reconcile detects the absent BM25 path and repairs it when the active chunker emits content within the incremental size ceiling. Text that yields no chunks remains a no-op. Paths missed by the walk are retained unless current policy excludes them or capability metadata confirms NotFound. The BM25 schema stays at version 3, preserving vector shards.

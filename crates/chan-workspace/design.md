...
`sqlite` (rusqlite, bundled) backs the per-workspace graph in sidecar state. Schema lives in `nodes` (Markdown documents), `edges` (links + tags + mentions), `headings` (in-file anchors), and `text_files` (`rel_path`, `mtime`, `size` stamps for non-Markdown text, plus `no_index_entry`). A `nodes` row carries `rel_path`, `kind` (`file` / `contact`), `mtime`, `size`, `title`, `basename`, `emails` (lowercased addresses extracted from contact bodies), `aliases` (top-level `aliases:` frontmatter, used by the mention resolver to map `@@<alias>` to a contact file without re-parsing frontmatter per query), and `no_index_entry`. Both `no_index_entry` flags record that the content seen at the stamped `(mtime, size)` puts nothing in the search index, because it chunks to nothing or its bytes do not decode as UTF-8, so reconcile reads such a file once to learn that and not again at that stat. The incremental writers set the flag from the chunk count they just indexed or from the decode failure; the full rebuild's rows leave it unset, since its search build reads each file separately and can fail per file. Edge rows are `(src, dst, kind, anchor)` with the anchor in the primary key so one file can link the same target via two distinct anchors.

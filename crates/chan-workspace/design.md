...
```
  ...
  - `Workspace` is one registered directory. Holds the writer lock for its lifetime; cheap reads (search query, graph traversal) do not contend. Its filesystem face is a crate-private `RootedFs` core: the display root, its canonical form and unix identity, the cap-std `Dir` handle every user-path operation routes through, and the transfer ceiling that bounds opaque-byte writes. Search and graph sidecars are metadata-opened during construction; `OnceLock` retries on first use only when that probe failed. Report state stays lazy.
...
The filesystem core is a crate-private `RootedFs` value held by `Workspace`: the display root, its canonical form and unix `(dev, ino)` identity, the cap-std `Dir` handle, and the transfer ceiling for opaque-byte writes. Every user-path primitive (`classify_workspace_path`, `ensure_writable`, `write_atomic_stream`, the read/write/stat/list family, `rename`, `copy`, `resolve_free_name`, physical-path resolution, and `ensure_root_available`) executes inside `RootedFs`; the `Workspace` methods delegate to it and layer the metadata-coupled operations (trash-backed `remove`, link-rewriting rename, drafts, sessions, index and graph maintenance) on top.

The same core backs the public, metadata-free `MiniWorkspace`: one capability root plus a protected canonical start directory, opened without registry rows, workspace locks, sidecar metadata, indexing, or a graph. It speaks a stricter wire dialect (UTF-8, POSIX-relative, absolute paths refused rather than re-rooted, `""` only for root queries), shows every ordinary entry including dotfiles and control dirs while skipping non-UTF-8 names, keeps symlinks inert (visible in listings, refused by every read, write, copy, move, and delete), deletes only regular files and empty directories (`DirectoryNotEmpty` / `ProtectedPath` are typed refusals), and moves and copies without link rewriting: no clobbering (the committing rename itself refuses an existing destination, see `no_replace`, so one created after the check is kept and answered as a conflict), whole-tree preflight, cross-device moves through a uniquely named temporary sibling with the source removed only after the destination is complete. A write through a `MiniWorkspace` is an external write to any real `Workspace` covering the same physical file.
...
Two gates guard the text-class APIs:
  ...
  - **Indexer gate** (`fs_ops::is_indexable_text`): true only for `FileClass::EditableText` (markdown-class `.md` / `.txt`). Used by the indexer, graph rebuild, link-rewrite on rename, and reindex-after-restore. A `.py` is editable but **not** indexable so `#include` doesn't read as a `#tag`. Within the indexable set, `fs_ops::is_markdown_file` narrows `#tag` / `@@mention` token extraction to `.md` only: `.txt` indexes for full-text and carries a separate stat row for reconciliation, without a document node, headings, or outgoing graph edges. Inbound links from Markdown notes remain visible as backlinks.
...
`Workspace::read_bytes_bounded` opens and fstats one regular file and returns `BoundedFileReader`, a same-thread iterator that reads 64 KiB chunks from the open handle on the consumer's own thread. The byte window is frozen from the open-handle stat: growth after open is excluded from the stream, and a mid-stream shrink is an error rather than a short success. `BoundedFileReader` never allocates a whole-file buffer; dropping it closes the handle.
...
Workspace renames refuse an occupied destination with `PathAlreadyExists` before mutation. A destination inside the source tree is refused with `DestinationInsideSource` before creating destination parents or updating links, the index, or the rename log. A destination that identifies the source itself remains valid for same-path and case-only renames: Unix compares device/inode identity, and Windows compares native canonical paths after the capability-relative metadata checks. The shared rename/copy containment guard follows the deepest existing destination-parent ancestor through the capability handle. Unix compares source device/inode identity with each physical ancestor of that canonical path; Windows compares native canonical paths component-wise. It also covers standalone moves, copies, and cross-device moves. When the preflight found the destination absent, the rename commits with a rename that refuses an existing destination (see `no_replace`), so a file or directory created after the check is kept and the rename is `PathAlreadyExists`; the cases where the destination is the source itself (a same-path or case-only rename, and on Unix a hard link to the source) commit with a plain rename. The preflight does not exclude an external process replacing a component of the destination path before the mutation syscall.

Link-rewriting directory renames run the filesystem preflight before collecting their file mapping. The mapping walks only the source subtree, retaining root-relative paths and the listing skips. A subtree exceeding `LIST_TREE_LIMIT` is refused before moving anything; sibling entries consume no budget. A source with an in-root symlink component is walked through that component, so its descendant files appear in the mapping.

Directory copies preflight every included source entry for readable regular files, directories, and UTF-8 names, then copy into a temporary sibling and rename it to the final destination only when complete, with a rename that refuses an existing destination (see `no_replace`), so a destination created while the copy ran is kept and the copy is `PathAlreadyExists`. Failure removes the stage, reporting any cleanup failure; the source is untouched. The returned created-file list names final paths. Both facades share the preflight, the stage (removed on failure or unwind unless published), the publishing rename, and the cleanup helpers; the workspace copy skips `.chan`, `.git`, and `.hg`, while standalone copies retain ordinary control directories.
...
`read_text_with_stat` and `write_text_if_unchanged` are the optimistic-concurrency pair the editor uses to detect external edits. The editor reads `(content, stat)`, the user types, and the save round-trips through `write_text_if_unchanged(rel, Some(stat.mtime_ns), expected_disk, new_content)`. The token is the nanosecond mtime so two edits within the same wall-clock second still conflict on filesystems with sub-second mtime. Because a filesystem timestamp does not advance on every write, a matching mtime is additionally verified against the last-observed disk bytes when the caller passes `expected_disk`; callers holding no such belief pass `None` and get the mtime test alone. If another process (terminal, sync daemon, second pane) has since modified the file, the write fails with `WriteConflict { current_mtime_ns }` and the editor prompts to reload, merge, or overwrite. `write_text` (no CAS) remains for chan-workspace's own reindex helpers, bulk imports, and LLM-tool calls where last-write-wins is the intent. Residual race: between the mtime check and the atomic rename a foreign write can land; the watcher event for the foreign change fires on the next dispatch and the editor re-prompts.
...
Shared draft promotion refuses targets within the source draft before mutation; workspace promotion additionally refuses the entire configured drafts directory. Containment compares the canonical source directory with the target's deepest existing canonical ancestor, including symlink aliases. Workspace target resolution rejects parent traversal before this check.

Creating an existing draft returns `PathAlreadyExists`, carrying the workspace-relative draft path or the standalone draft name. Draft-create routes retry name allocation by this variant.

Merging a draft into an existing directory copies into a temporary sibling and tracks each child moved into the target. If a move or target sync fails, it restores those children to the stage in reverse order and removes the stage. The error names any child that could not be restored and any stage cleanup failure. The original draft remains intact on failure; unrelated target entries are untouched. A target that is itself a mount point can reject the sibling-to-target move with a cross-device error.
...
Trash entries expose five stable fields: opaque monotone `id`, workspace-relative POSIX `original_path`, Unix-seconds `deleted_at`, `is_dir`, and `size` (file length or recursive directory total).
  ...
  - **Recovery marker**: when the same-fs move has already taken the content out of the workspace, the metadata write fails, and moving the content back fails too, the entry keeps its payload beside a `recover-me.txt` marker naming the original path. That payload is the only copy, so the sweep does not reclaim it, and `trash_empty` leaves it in place and counts it as `kept_for_recovery` in its `TrashEmptyReport`. Only `trash_purge` with that entry's id deletes it. The entry has no metadata, so `trash_list` does not show it.
...
Config writers serialize snapshot, persistence, and adoption through a separate transaction mutex, including model/exclusion setters, vector clearing, and build stamps. They save a candidate before replacing the live config; a failed save leaves readers on the prior config. Readers hold only the short-lived config mutex, never the transaction mutex, and remain available during the file write. Model changes retain the transaction mutex through vector invalidation.

The config and lazy embedder mutexes are never held together. Lazy initialization snapshots the model before locking the embedder; the post-build stamp snapshots the dimension before locking the config. Model loading therefore cannot form a lock cycle with the build stamp or config readers. Initialization checks the vector epoch after acquiring the embedder lock and retries a snapshot invalidated by a concurrent model switch or clear.
...
Each per-file shard under `embeddings/` carries its own `FORMAT_VERSION` (currently 2), checked per shard in `load_all`; a version mismatch or decode failure warns with the shard path and skips only that shard. The index still opens for BM25 and healthy vector search. A later `replace_file` rewrites the shard for a live source; every complete build, including BM25-only builds, prunes orphan `.bin` filenames against hashes of the visited paths, so even an undecodable orphan is reclaimed. Directory-level I/O failures still fail vector-store open. This is intentionally independent of the schema-version wipe path: a shard-format bump should NOT invalidate BM25 segments or other shards, only the shards that actually need re-encoding. The `--no-default-features` build excludes the entire embed path including the skip check; the `embeds_reused` field stays in `BuildSummary` and reads zero.
...
The same policy Arc is sampled by bootstrap, filtered listing, graph, BM25, reconcile, report, Linux watch registration and dynamic registration, and watcher dispatch. Full walks do not follow symlinks and stay on the workspace root filesystem; Linux registration compares every candidate directory's device identity with its `WatchRoot` before registering or descending. A gitignore negation keeps a configured-excluded directory traversable only when the pattern's fixed prefix can match within that directory; an unrelated negation cannot widen every configured exclusion. `Workspace::set_excluded_dirs` persists the setting, advances the shared recovery generation, swaps a new immutable policy, and requests reconcile. Watch registration observes replacements, resets registrations when a replacement races a retry, and discards queued directory work tagged with an older generation. The editor-visible raw `Workspace::list_tree` and `Workspace::list` APIs remain unfiltered so a user can deliberately open a file inside a noisy directory.

### Graph

`sqlite` (rusqlite, bundled) backs the per-workspace graph in sidecar state. Schema lives in `nodes` (Markdown documents), `edges` (links + tags + mentions), `headings` (in-file anchors), and `text_files` (`rel_path`, `mtime`, `size` stamps for non-Markdown text, plus `no_index_entry`). A `nodes` row carries `rel_path`, `kind` (`file` / `contact`), `mtime`, `size`, `title`, `basename`, `emails` (lowercased addresses extracted from contact bodies), `aliases` (top-level `aliases:` frontmatter, used by the mention resolver to map `@@<alias>` to a contact file without re-parsing frontmatter per query), and `no_index_entry`. Both `no_index_entry` flags record that the content seen at the stamped `(mtime, size)` puts nothing in the search index, so reconcile reads such a file once to learn that and not again at that stat. On a `nodes` row the only reason is content that chunks to nothing; a `text_files` row carries that reason or bytes that do not decode as UTF-8, since a Markdown file that does not decode is forgotten rather than recorded. The incremental writers set the flag from the chunk count they just indexed or, for a `text_files` row, from the decode failure; the full rebuild's rows leave it unset, since its search build reads each file separately and can fail per file. Edge rows are `(src, dst, kind, anchor)` with the anchor in the primary key so one file can link the same target via two distinct anchors.
...
Every event carries the policy `WorkspaceGeneration` sampled at dispatch, and stale generations are rejected by the graph and report consumers. `is_dir` preserves directory identity even for Linux `MOVED_FROM` events whose source has already vanished. Rename events expose the Linux inotify correlation `cookie`; it remains `None` on other backends and synthetic events. A rename's paths are assigned by the backend's rename mode, never by position: a paired rename fills both slots, a single-path `From` fills the source, and a single-path `To` (an inotify `MOVED_TO` with no paired `MOVED_FROM`, or either half of a Windows rename) fills the DESTINATION, so a file moved into the workspace is not read as a file that vanished. macOS FSEvents names no end at all, so its single path stays in the source slot and consumers stat it; only a genuine source path deregisters a watched subtree. A path that cannot be mapped into any watched root is emitted as a pathless `ProviderError` loss event so consumers reconcile instead of silently dropping an unknown mutation.
...
`Workspace::start_graph_indexer(debounce_ms)` returns a `GraphIndexer` handle that owns a watcher subscription and one worker thread. Nothing outside chan-workspace calls it; chan-server runs its own indexer (`crates/chan-server/src/indexer.rs`). `DEFAULT_DEBOUNCE_MS` is 150.
  ...
  - `Removed` events and the source side of `Renamed` skip the debounce: a stale graph row pointing at a missing file is the user-visible failure mode, so deletions flush immediately. Directory events forget every graph and search row at or below the source path through the same journaled mutation boundary. A `Renamed` that names a single path in the source slot can name either end (FSEvents reports each end of a move alone), so that path is checked first. It is present only when every component, from the root down, is listed byte for byte by the directory above it, since on a case-insensitive volume a lookup also finds the old spelling of a case-only rename of the file or of a directory above it (`Workspace::parent_lists_name`). One that is not present is forgotten as a source, and one that is present is indexed by what it is on disk rather than by the event's directory flag, which can be stale by then. A file is scheduled like a create; a directory asks for a `Workspace::reconcile`, because the files under it raise no events of their own, and the asks debounce like a path, so a burst of them runs one reconcile.
  - `ProviderError` and path-less events (the watcher's "scope unknown" signal) clear everything pending and trigger a full `Workspace::reconcile`, the same convergence path used for cold-open catch-up.
  - Counters (`pending_count`, `indexed_total`, `forgotten_total`, `reconciles_total`) back the status surface. Dropping the last clone of the handle (or calling `stop()` on any clone) tears down synchronously: watcher first (closing the channel), then the worker joined. Of the indexer's own state the worker holds only the stop flag and the counters, so no reference it holds keeps the part the handles share alive past the last clone.
...
The per-workspace code/SLOC/COCOMO report wraps `chan-report`'s incremental `Index`. chan-workspace owns the persisted JSONL, the load-else-rescan open path (any load error, including a missing file, schema mismatch, or partial write, falls back to a full scan that replaces the bad file on the next flush), and a dedicated writer thread that debounces flush signals (500 ms window) and atomic-writes the JSONL. The presence of a persisted report file records a refresh obligation when the workspace opens, even when loading that file later fails; the claimant of the open's first recovery pass rescans it after any successful replay, reconcile, or full rebuild and clears the obligation only after the scan succeeds. A watcher loss on a warm report records the same obligation before the watcher's user callback runs, because the incremental index cannot tell which rows the lost events touched, so the claimant of the recovery pass the loss requests rescans the report through the same path; the built-in graph indexer reconciles a loss in place and requests no pass, so a host that drives only that indexer leaves the obligation owed until some pass runs; a loss that arrives while a rescan runs leaves the obligation owed when that rescan ends, since the scan may predate the lost events. Watch events fan into the in-memory index before the user's callback runs, so a handler that immediately calls `Workspace::report()` sees the change; `Unchanged` outcomes do not schedule a write. The walk applies the same excluded-dirs policy as the index walk so a source-tree workspace doesn't roll up its dependency trees.
...
Email extraction decodes Markdown backslash escapes of accepted local-part punctuation, including underscores, before lowercasing and deduplication. The emitter keeps its escaped on-disk form. Stored email projections refresh on the next per-file ingest or reindex; changing extraction does not force a rebuild.
...
`metadata_archive` exports and imports the per-workspace sidecar state as a zstd-compressed tar so search/graph/report caches and session blobs can move between machines (or serve as a backup) without re-deriving them. The archive has a versioned manifest plus payloads keyed by the same canonical-path slug + sha256 scheme as the metadata key. It includes cache/session sidecars and excludes locks, machine-local tokens, trash, and transient staging/temp files. Entry paths are validated on both sides (`validate_archive_entry_path`) so a crafted archive cannot write outside the payload root, and extraction stops at `MAX_ARCHIVE_ENTRIES` (10,000) tar entries and `MAX_ARCHIVE_PAYLOAD_BYTES` (4 GiB) of decompressed payload, metered on the bytes actually read, with `ChanError::ArchiveLimit`; the staging directory is removed and the live metadata is untouched. The manifest read stops at `MAX_ARCHIVE_MANIFEST_BYTES` (1 MiB), and both it and extraction read the decoded stream through a cap (for extraction, the payload budget plus the manifest plus 8 KiB per entry) that refuses the archive by name one byte past it. A GNU long-name, long-link or PAX extension header is still read into memory whole, so one can cost memory up to that decoded cap; the archive comes from an authenticated principal. The manifest records the archive format version, the producing chan version, and per-workspace identity so import can match payloads to registry rows.

Export snapshots the graph through a read-only SQLite connection using `VACUUM INTO`, producing one staged database without an archived WAL. The BM25 snapshot reads one atomically published `meta.json`, parses those captured bytes with Tantivy, and copies only that commit's immutable segment and deletion files. A segment lost to concurrent garbage collection restarts the snapshot, up to three attempts; exhaustion returns an export error. The staged `.managed.json` lists exactly the copied files for the imported writer's garbage collection. Snapshots and the captured recovery records use temporary directories under the metadata staging root and hold no workspace or Tantivy writer lock, so an open workspace continues indexing during export.

Graph and BM25 snapshots are sequential and can describe different moments. Between them the export reads `graph/pending_writes.json` and `graph/rebuild.inprogress` and archives those bytes, or their absence, in place of the live files: a per-file mutation records its journal entry before its graph commit and clears it after its index commit, and the rebuild marker brackets a full rebuild the same way, so a write the graph snapshot already carries either still holds its entry at capture time or has already committed its chunks into the index snapshot. Vector shards, config, the rename log, report, and sessions share no snapshot transaction with either store. On import, the next open schedules journal replay, reconcile for populated stores, or a full rebuild for detected incomplete state. Reconcile compares disk metadata with graph rows, so it does not guarantee repair of every graph/BM25 timing difference when both stores are populated and the graph already matches disk. An explicit rescan rebuilds both from user files.
...
A single-file copy, through `Workspace::copy` or `MiniWorkspace::copy_plain` alike, connects `BoundedFileReader` directly to the bytes-mode atomic sink, which writes the file under the destination's own name inside a private stage directory beside the destination; the staged file is then renamed onto the destination with a rename that refuses an existing one (see `no_replace`), so a file or directory created after the absence check is kept and the copy is `PathAlreadyExists`. A new copy therefore inherits the byte budget, incremental UTF-8 validation when the destination is editable text (the staged path classifies as the destination does), a commit that never crosses filesystems, and stage cleanup on source, sink, or publish failure. The source's open-handle size is checked against the sink budget before the first chunk is accepted.
...
Search and graph sidecars are metadata-opened during workspace construction so readiness can be classified without visiting the workspace tree. Report state remains lazy unless a persisted report needs startup recovery. Explicit reindex calls are synchronous; the startup worker runs the open plan by default, while an installed recovery driver can claim the same pass. A persisted-report rescan is an obligation on that pass rather than on either claimant: ordinary `finish_recovery` completion requeues the pass while the obligation remains owed. The served coordinator may instead finish after its bounded refresh retry is exhausted, retaining the obligation for the next independently requested recovery pass. The built-in watcher/indexer remains optional and uses the same scope policy as the cold paths. Graph writes serialize through the graph writer connection; graph reads use the reader pool. Report state is opt-in, watcher-fed before user callbacks run, and persisted through chan-report JSONL. `report_if_available` is the non-scanning read path: it returns a warm snapshot or loads a valid persisted JSONL and otherwise returns `None`; unlike `report()`, it never falls back to a filesystem scan.

One workspace `write_serial` mutex is the derived-state mutation boundary. It covers per-file graph/BM25 index and forget, pending-journal replay, reconcile as one unit, full rebuild, and warm report watcher updates. Nested bulk paths call lock-assuming per-file helpers, so they do not reacquire the non-reentrant mutex. Incremental `index_file` stats included `.md` and `.txt` files before this lock and declines files larger than `TEXT_WRITE_LIMIT`; lock-owning callers repeat the same gate before reading. Search, graph, report snapshot reads, filesystem reads, and bounded file streams never take this lock.
...
Push vs. pull, and how the signals relate:
  ...
  - `begin_recovery` moves pending work into the single active slot and `finish_recovery` either completes that generation or requeues it without inventing a new one. `Workspace::is_reindexing()`, `needs_rebuild()`, and `needs_replay_writes()` are projections of the coordinator.
  - chan-server requests recovery before enqueueing the returned `WorkspaceGeneration`. Its rebuild coordinator drains queued requests to the newest required generation, claims and finishes each pass through the workspace coordinator, and exposes idle only after the completed generation covers that requirement with no pending work. `Workspace::run_full_rebuild_pass` is the narrow seam for an already-claimed server pass. A trigger during a rebuild therefore requires one follow-up pass, repeated triggers coalesce onto it, and the rebuild cooldown remains a floor before that follow-up starts. A pass whose action fails is requeued through `finish_recovery` and not claimed again until one cooldown later, whatever its action, so while it keeps failing `Workspace::readiness()` stays `Recovering` with that pass pending and the server's index status reads `error`. A claimant that goes away mid-pass must still finish the pass it claimed: the server's coordinator carries its claim with the blocking run and requeues it when the run ends, because `begin_recovery` hands out one active pass at a time and a pass nobody finishes holds that slot for good.
  - `Workspace::readiness()` projects one coordinator snapshot into the serde-tagged `WorkspaceReadiness::{Ready, Recovering}` value consumed by server, desktop, and CLI. Recovering carries the required, completed, active, and pending generations plus the dominant required action, without exposing the coordinator mutex or worker. Server status, indexing-state, and preflight responses carry the same value. Content and unified workspace queries conservatively return explicit not-ready envelopes with no derived rows during recovery; no readable snapshot generation is claimed, because graph, BM25, and report do not persist one shared completion proof.
  - `Workspace::open` returns a metadata-only `RecoveryPlan` with the durable marker/journal inputs, graph and search readiness, and policy generation. `Library::open_workspace` starts the owned recovery worker only after construction returns. A warm populated pair schedules reconcile; a stale rebuild marker or graph-populated/search-empty pair schedules full rebuild; a pending journal schedules replay; an empty cold pair skips the tree. Persisted-report presence separately seeds a refresh obligation and at least a reconcile pass; a later watcher loss on a warm report seeds the obligation again (see "Report"). The obligation survives a stopped worker, a failed refresh, and an unwinding refresh. Ordinary `finish_recovery` completion requeues the pass until a claimant clears the obligation by rescanning successfully. chan-server's coordinator permits at most one retry caused by a refresh failure per activation, then completes that generation with the obligation still owed. An activation starts with a wake the idle coordinator receives and absorbs every wake it drains while it runs, including the one its own retry sends. The bound never clears the obligation, so while the refresh keeps failing every later activation that claims a pass spends up to one extra pass on it: the bound limits coordinator work per wake, not over the life of the process. A host that stops the startup worker or relies on built-in watcher-overflow reconciliation must retain or install a recovery driver if it needs an immediately requeued pass to converge. The durable files remain the cross-process authority, and `RecoveryStatus` stays pending/active until the derived-state pass finishes.
  - `Workspace::reconcile()` is the diff-based recovery path. Walks the live tree, compares each editable-text file's `(mtime, size)` against its graph document or text stamp row, and emits journal-bracketed `index_file` / `forget_file` calls only for the deltas. Unchanged indexed files are skipped, so reconcile costs O(N) stat + per-file indexing for changed files or missing text entries. A matching graph row without a BM25 path, document or text stamp, is probed with the active chunker, within the existing incremental size ceiling, unless the row already records that the index holds nothing for it: content that produces chunks is indexed again, while empty, whitespace-only and frontmatter-only content is marked `no_index_entry` and stays unchanged, read once rather than on every pass. A failed repair probe is logged at debug and retried on a later pass. Use cases: cold open after offline edits, watcher-overflow recovery (inotify `IN_Q_OVERFLOW` / FSEvents coalesce-loss), post-`replay_pending_writes` sanity. The diff compares `(mtime, size)` tuples against the graph's stamped row; a same-mtime-different-size rewrite is caught via the size delta. Rows that carry `size = NULL` (stamped before a size was recorded) fall back to mtime-only for that row, and a subsequent `index_file` backfills size. A file needing indexing that the pass cannot read is logged at warn and listed in `ReconcileReport::failed`, and the pass carries on: the same policy the full graph rebuild applies, so one bad file in an imported tree cannot strand every file after it or stop the deletion pass from running. A non-Markdown text file whose bytes do not decode as UTF-8 is recorded as seen at its `(mtime, size)` with `no_index_entry` set, so later passes count it unchanged until it changes on disk; a Markdown file that does not decode, or any file that cannot be opened or was removed under the walk, is dropped from both backends and tried again on the next pass. `replay_pending_writes` degrades such an entry to a forget for the same reason, so it stops seeding a failing replay on every open. The deletion pass unions document rows, text stamp rows, and the search index's known paths. A path missed by the walk is forgotten only if current policy excludes it, capability-relative `symlink_metadata` returns NotFound, or any of its components is no longer listed byte for byte by the directory above it, which is how a spelling that a case-only rename of the file or of a directory above it replaced is told apart on a case-insensitive volume where the lookup still succeeds (each directory is listed at most once per pass); existing paths and other metadata or listing errors retain graph and index data with a debug log. This confirmation covers Markdown, text and index-only paths, protecting files missed by walk errors or filesystem boundaries while reclaiming confirmed deletions and exclusions. A missing text stamp triggers one ingest, then unchanged text files are skipped. A failure of the graph or the search index itself still aborts the pass.
...
Two distinct concurrency primitives:
  ...
  - `lock::probe_foreign_holder` is the non-blocking read-side probe for UI and CLI status, and it answers with a three-state `ForeignHolder` rather than a `bool`, because a probe can fail to establish anything. It does not acquire for ownership, does not steal, and leaves `is_free` and acquisition unchanged. `Present` means the OS reported the writer lock contended by a live, torn-record or path-mismatched holder; contention is itself the observation, so a held lock whose record cannot be read is still `Present`, which establishes that the lock is held but not by whom. `Absent` covers a free lock, this process's own holder, and a holder whose lock-body record proves it dead. `Unknown` means nothing was established: the lock file could not be opened, as under file descriptor exhaustion, or the lock could not be tested for a reason other than contention. It carries a diagnostic reason and is never reported as a holder.
...
Every target uses `.chan` under the OS-provided home directory for config, state, and cache alike. `CHAN_HOME` overrides the directory directly. If no home directory resolves, chan falls back through a chain that never fails to pick a directory: first `/var/tmp/chan-<uid>` on Unix, created `0700` and accepted only as a real directory owned by this uid (its mode is tightened through a no-follow descriptor), or `C:\ProgramData\chan` on Windows, refused if it is a reparse point or not a directory; then a fresh mkdtemp directory, `0700` on Unix, under `/var/tmp` and the temp dir; and last `.chan` under the working directory, created `0700`. That last resort is the one place chan keeps its home in the working directory: a checkout used that way gains an untracked `.chan` there, holding the config and its token, and when the working directory cannot be read the path is relative and the log says so. `/var/tmp` is world-writable, so the first step's checks are what stop another local user preparing chan's home. The choice is made once per process and logged with every refused location, and state kept in a fresh directory does not survive a restart. One platform-independent selector derives the home-relative path or invokes that fallback; `state_dir` and `cache_dir` delegate to `config_dir`.
...
## 8. Schema versioning
  ...
  - **Graph DB**: `PRAGMA user_version`. Migrations are idempotent and applied on every `GraphView::open`. Current version: 8 (v2 added `basename`, v3 added `emails`, v4 added staging tables for resumable reindex, v5 added a `size` column on `nodes` + `staging_nodes` so `Workspace::reconcile` can detect same-mtime-different-content rewrites that a mtime-only diff would miss, v6 added `aliases` so the mention resolver can map `@@<alias>` to a contact without re-parsing frontmatter per query, v7 added the separate `text_files` stat table, v8 adds `no_index_entry` on `nodes` and `text_files` so reconcile can tell content the index legitimately lacks from an index entry that went missing without reading the file on every pass). Each `v < N` block wraps its schema change and the `PRAGMA user_version = N` bump in a single `unchecked_transaction` so a crash mid-migration leaves the DB at the previous version with intact data. A `column_exists` helper guards the `ALTER TABLE ADD COLUMN` calls so a crash before commit, which leaves the column physically added but `user_version` at the prior value, is safely re-run on the next open without `duplicate column name` errors.

The graph rebuild collects readable text-file stamps from its walk and replaces `text_files` in the same transaction as the Markdown staging swap. Text stamps do not create document nodes. An older binary skips migration blocks for unknown higher `user_version` values and ignores the additive table and column; a newer open reconciles changed, missing, and index-only paths again. A graph row is not proof of an index entry, for a document or a text stamp: the rebuild's search build reads each file separately and can fail per file, so the rebuild's rows leave `no_index_entry` unset, and reconcile reads a row's file once when the index lacks it, repairing content the active chunker emits within the incremental size ceiling and marking content that yields nothing so it is not read again at that stat. A graph from before the column opens with the flag unset on every row, which costs the same one read per index-less row on the next reconcile. Paths missed by the walk are retained unless current policy excludes them or capability metadata confirms NotFound. The BM25 schema stays at version 3, preserving vector shards.
  ...
  - **Vector shards**: each per-file file under `embeddings/` carries its own `FORMAT_VERSION` (currently 2; the format includes the `body_hash` checkpoint field for embed-phase skip on partial rebuilds). The version is checked per shard in `load_all`; a mismatch or decode failure warns and skips the shard, leaving BM25 available. The next successful embedding replaces it; complete builds also reclaim orphan shard filenames even when their payload cannot be decoded. This is intentionally independent of the schema-version wipe path: a shard-format bump should NOT invalidate BM25 segments or other shards, only the shards that actually need re-encoding.

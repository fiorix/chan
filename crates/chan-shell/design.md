...
- Workspace-only (refused on a standalone terminal, which has no workspace): `open`, `graph`, `search`, `export`, `terminal new --path`, every `session` subcommand, and every `terminal team` form including `--script`. The refusals share one message family via `workspace_only_refusal`, and `cs open` additionally points at `chan serve PATH`.

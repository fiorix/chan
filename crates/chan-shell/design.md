...
In scope:
  ...
  - The control-socket transport: connect, write one JSON request line, read one JSON response line, over a per-user Unix-domain socket on unix and a named pipe on windows. Survey and requester-side handover calls retain their write half until that response so EOF can cancel the parked request; `cs tunnel` is the long-lived exception that continues after its acknowledgement (below).
...
Each family resolves its target a different way: window-id push, registry lookup, a parked SPA round-trip, the desktop bridge, or teardown, and only the SPA-blocking family can answer `Timeout`. The breakdown:
  ...
  - Desktop window lifecycle through the in-process Tauri bridge the embedded server installs. `cs window new` / `open` / `rm` / `hide`. A standalone `chan serve` has no desktop attached and refuses them.
...
`send_control_request` is platform-neutral over a small `transport` module, the only `#[cfg]`-split surface. On unix it connects a `UnixStream`; on windows it opens a named-pipe client (retrying `ERROR_PIPE_BUSY` and a momentarily-absent pipe under a bounded deadline so a genuinely-missing server still fails fast). Above that split the ordinary protocol is identical: serialize the request, append a newline, write it, half-close the write side, then read one response line. The `\n` frames the request, so the half-close is belt-and-suspenders rather than load-bearing, and a server that has already replied and closed can make it fail (macOS answers `shutdown(2)` with `ENOTCONN` once the peer is gone), so the client ignores a `NotConnected` refusal and reads the reply that is already queued; any other shutdown error is reported. `send_control_request_streaming` skips the half-close and retains both halves. `cs tunnel` keeps its returned `TunnelSession` after the acknowledgement. `send_control_request_held` uses the same path for a flagged survey or requester-side handover, then drops the retained connection after its first response. The server treats EOF as cancellation only when the request carries `cancel_on_eof`, so an omitted field preserves the ordinary half-close behavior under version skew.
...
- Filesystem-dependent: `open PATH` and `terminal new PATH` need a workspace window or a standalone terminal whose host serves a filesystem. PATH is positional. In a standalone terminal, both resolve through the served filesystem's capability root; without that filesystem, `cs open PATH` points at `chan serve PATH`. In a workspace window, `terminal new PATH` must resolve inside the workspace root, and a file targets its parent directory; `open PATH` can route outside that root.
- Workspace-only (refused on a standalone terminal, which has no workspace): graph-link `open`, `graph`, `search`, `export`, every `session` subcommand, and every `terminal team` form including `--script`. The refusals share one message family via `workspace_only_refusal`.
- Desktop-only (a separate axis): `window new`/`open`/`rm`/`hide` need the chan desktop app. `tunnel` runs from any terminal window but needs a devserver or desktop HOST (a standalone `chan serve` server refuses it) plus a chan-desktop-opened window to answer the trigger; a browser-only viewer times out the ready wait (exit 124) with a message naming the desktop as the only capable client.

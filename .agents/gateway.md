...
- **The devserver id is the PAT digest.** `chan_tunnel_proto::gateway_assertion::devserver_id_from_token` is the lowercase-hex SHA-256 of the raw PAT (same digest as the stored hash, hex-encoded). One token identifies one devserver; this 64-char string is the cross-service handle the tunnel registry keys on and the `drv` claim carries. The raw PAT is returned once by identity, then the devserver sends it to devserver-proxy in the tunnel Authorization header (TLS except for loopback development), and devserver-proxy forwards it to identity's protected internal listener in a request authenticated with `IDENTITY_INTERNAL_TOKEN`; identity persists only the digest.

...
`cs terminal team new <dir> --config <file>|--stdin [--brief FILE] [--mcp-env on|off] [--tabs] [--script]` validates the config, materializes the team directory, and spawns the members; `cs terminal team load <dir>` respawns an existing team. `--script` emits the whole flow as a paste-and-run shell script instead of executing it. The script carries the team directory as the CLI resolved it at generation time (workspace-relative) and joins it to `$PWD` once, at the top: it prints that resolved directory, writes the tree under it, and points every identity poke at it, so the tree and the pokes cannot disagree wherever the script runs. A scripted team therefore lands under the directory the script is run from. From the workspace root that is where the direct form puts it and where `cs terminal team load <dir>` looks; from anywhere else the members are still pointed at the right place, and `load` is not.

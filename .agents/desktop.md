...
The desktop app verifies update bundles with a minisign signature. The production pubkey is embedded in `src-tauri/tauri.conf.json` under `plugins.updater.pubkey`. Matching private key material lives outside the repo in the release owner's secret store. Two payload shapes are signed with it at release time, each shipping with a detached `.sig` release asset: the macOS `Chan_<v>_aarch64.app.tar.gz` (the `macos-desktop-artifacts` job) and each Linux `Chan_<v>_{amd64,aarch64}.AppImage` itself (the `linux-desktop-artifacts` job, which fails fast when `TAURI_SIGNING_PRIVATE_KEY` is absent, like the macOS job). The Windows NSIS installer is not an updater payload.
...
4. Old installs that never picked up the bridge release will fail to verify new-key-signed bundles and stall on their last good version until the user manually reinstalls. Plan the bridge window for how long you're willing to support that tail. A Homebrew cask install recovers with `brew reinstall --cask fiorix/chan/chan-desktop`: the cask is `auto_updates` so brew never refreshes it on its own, but every GA rewrites it to the current DMG. A Linux AppImage install recovers by downloading the current AppImage over the old file by hand.

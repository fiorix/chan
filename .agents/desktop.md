...
The Windows CLI, desktop binary, NSIS plugins and installer are signed remotely through SSL.com eSigner by `src-tauri/scripts/windows/sign.ps1`, which `release.yml` (job `windows-artifacts`) and the `release-desktop.yml` dry run call directly and through tauri's `signCommand`. It runs SSL.com CodeSignTool, pinned to a GitHub release, and the tool runs the JDK bundled in its archive rather than the runner's trust store. That JDK's truststore predates `SSL.com TLS RSA Root CA 2022`, the root SSL.com's signing endpoint chains to, so both workflows' `Install SSL.com CodeSignTool` step imports the committed [`ssl-com-tls-rsa-root-ca-2022.pem`](../desktop/src-tauri/scripts/windows/ssl-com-tls-rsa-root-ca-2022.pem) into it with the JDK's own `keytool`, after checking the file against its pinned SHA256 fingerprint. CodeSignTool exits 0 even when a sign fails (a TLS handshake it cannot complete, for one), so its exit code is not a signal: `sign.ps1` reads the Authenticode signature back with `Get-AuthenticodeSignature` for every `.exe`, `.dll` and `.msi` it signs and throws unless the status is `Valid`. A trust or credential break therefore fails at the first signed file, not at the later `Verify Windows Authenticode signatures` step.

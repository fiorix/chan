...
- Source-text assertions (a `?raw` import of a production module, `import.meta.glob` with `query: "?raw"`, a `node:fs` read of a source file) survive only for a build-time contract with no runtime seam: a property of the source tree that no mounted or behavioural test can observe, typically because it must hold for a module nobody has written yet. The allowed cases are:
  - `no_native_dialogs.test.ts` in the workspace app, the launcher and profile: no shipped module calls `window.alert`, `window.confirm` or `window.prompt`, which fail silently in the desktop WebView.
  ...
  - `hidden dashboards keep layout via visibility, not display:none` in `components/DashboardTab.keepAlive.test.ts` and `hidden editors keep layout via visibility, not display:none` in `components/FileEditorTab.keepAlive.test.ts`: a kept-alive tab that is not active is hidden with `visibility: hidden` over its full box (`position: absolute; inset: 0`), never `display: none`, so CodeMirror and the dashboard's index graph keep their layout geometry while hidden. jsdom lays out nothing, so no mounted test can tell the two apart.
  - `components/GraphPanel.keepAlive.test.ts`: a hidden graph tab keeps its size through `visibility`, never `display: none`, which would refit its canvas to nothing and lose the pan and zoom.
  - `components/Pane.surfaceTheme.test.ts`: `App.svelte`'s theme token blocks match any `[data-theme]` subtree, not only a pane, so a per-surface theme reaches its body; jsdom applies no stylesheet, so a mounted surface carries the attribute either way.
  - `components/contextMenuChrome.test.ts`: no rule on the pane element, in any state, scales it, since a scaled pane corrupts xterm's WebGL glyph atlas.
  - `editor/wysiwygLayerOrder.test.ts`: the z-index order of Wysiwyg's negative-z paint layers, or selections paint under the page fill.
  - `editor/decorations/blocks.test.ts`: Wysiwyg's rule for a decorated line's class reads each `--cm-md-*` variable the decorations set on that line (today `--cm-md-list-level` on `cm-md-list-hang`), or nested lists lose their indent.
  - `state/graphPalette.test.ts`: the literal CSS palette blocks in `App.svelte` and `GraphTuner.svelte` equal the TypeScript palette defaults.
  - `components/TerminalTab.font.test.ts`: the app entry imports `fonts.css` so the terminal face starts loading at boot, its `@font-face` src is relative, since an absolute src resolves to the launcher's `index.html` under a tenant prefix, and the family `fonts.css` declares is the family the terminal asks for. It reads `src/fonts.css` with `node:fs`; its reads of the bundled font and `OFL.txt` are shipped assets, not source.
  - `editor/ExcalidrawCanvas.test.ts`: the board's offscreen shell is `display: none`, since WKWebView leaks the island through `visibility: hidden`, and the island imports Excalidraw's stylesheet so the CSS rides the island's chunk, which vitest's dropped CSS imports cannot show.
  - `terminal/protocol.test.ts`, until a Rust test pins it: read with `node:fs` from `crates/chan-server/src/routes/terminal.rs`, the attach prelude sends the session frame first and the alt-screen prelude between the replay and ready, and a Resize frame resizes the PTY.

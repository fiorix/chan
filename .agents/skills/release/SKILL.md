...
6. **Platform verification.** Before GA close, run `make windows-cross-check` on the release candidate and require it green, then confirm the `release.yml` dispatch from step 4 is green for the same candidate. The release `make pre-push` gate is Linux-only and cannot see Windows or macOS breakage. The cross-check compiles and lints the CLI crate graph for `x86_64-pc-windows-gnu`; it does not link or smoke a Windows binary, and `ci-windows` remains the authority for that. The `publish=false` dry run remains the only macOS compile available off a macOS workstation. Neither platform check substitutes for the other.

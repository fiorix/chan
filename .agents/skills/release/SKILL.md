...
- The Nix npm-deps pin in `packaging/nix/chan-desktop.nix` (`npmDeps.hash`): regenerating `web/package-lock.json` invalidates this fixed-output hash, so re-pin it in the SAME commit (build once, copy the hash from Nix's mismatch error, or run `make nix-check` where Nix is installed). The Cachix publish job checks out the TAG, so a hash missed here cannot be repaired for that release afterward; v0.81.0 shipped without Cachix pins for exactly this reason.

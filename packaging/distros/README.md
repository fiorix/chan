...
Source packaging for Fedora COPR, Ubuntu Launchpad (PPA), and the Arch User Repository, plus the binary Homebrew tap: standalone `chan` and `chan-desktop`. COPR and Launchpad build from a vendored source tarball because their builders are offline; AUR users build from the tagged source with locked npm and Cargo dependencies; Homebrew serves the prebuilt macOS release assets. The public command surface is the root Makefile (`make distros-tarball`, `make copr-srpm`, `make copr-build`, `make copr-check`, `make ppa-source`, `make ppa-upload`, `make aur-check`, `make homebrew-check`).
...
- Publication pushes to the tap over SSH with the `HOMEBREW_TAP_DEPLOY_KEY` repo secret, the private half of a write deploy key on the tap repo. The auth probe expects the deploy-key greeting naming `fiorix/homebrew-chan`, so a key registered any other way fails the chain.
...
- `homebrew-auth` proves the `HOMEBREW_TAP_DEPLOY_KEY` credential against github.com before anything is pushed. `homebrew-validate` renders both definitions on an arm64 macOS runner, stages them as a local tap, runs `brew style` and `brew audit`, then really installs the cask and the formula and smokes `chan --version`; it gates publication. `homebrew-publish` copies the rendered files into a clone of the tap, pushes one commit covering both, and confirms the tap serves the released version through the GitHub contents API.

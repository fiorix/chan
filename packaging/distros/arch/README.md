...
**Automatic AUR publication is suspended.** The `aur-auth`, `aur-validate`, and `aur-publish` jobs are gated off by a literal `false` in their `if:` conditions while the Arch Linux [Active AUR malicious packages incident](https://archlinux.org/news/active-aur-malicious-packages-incident/) stands unresolved, because its restrictions include pushing package updates. A GA release publishes every other downstream target and pushes nothing to the AUR. Restore when that notice is superseded by an announcement stating both that the incident is resolved and that package pushes are permitted again; lifting is one deletion per job, the `false &&` line. A dispatch with `targets=aur` fails with a message naming the incident rather than selecting nothing and reporting success. Local validation below is unaffected, and the recipes are still built and smoked on every CI run.

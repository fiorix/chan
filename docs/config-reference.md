...
The server discovers what is installed on its own machine -- Windows PowerShell, PowerShell 7, cmd, Git BASH and every WSL distribution on Windows; `/etc/shells` plus `$SHELL` on unix -- so most machines need no `[[terminal.profiles]]` at all. Entries exist to override, hide, or add to that list, and are matched to a discovered profile by `id`:

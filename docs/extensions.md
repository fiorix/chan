...
A devserver running as the systemd user service on Linux (`chan devserver start`, `join` or `restart`, whose default backend there is systemd) runs with the `PATH` its unit records, and that `PATH` is also the one every terminal the service opens starts from, not only its extensions. `chan devserver restart` always rewrites `~/.config/systemd/user/chan-devserver.service`, and `start` and `join` do when the service is not running. A run from a terminal copies the `PATH` of the shell it runs in into an `Environment="PATH=..."` line, keeping absolute entries only: it drops relative and empty entries, every repeat of an entry after its first, and any entry containing `"`, `\`, a control character or bytes that are not UTF-8. A run with no terminal on standard input, such as a desktop connect script or any other script, keeps the `PATH` the unit already records and records its own only when the unit has none, and a run whose `PATH` has no usable entry keeps the recorded one too. So after adding a directory an extension needs, run `chan devserver restart` from a shell that has it, and from a plain one: a restart from inside an activated virtualenv or a `nix develop` shell records that environment's `PATH` for every terminal the service opens.

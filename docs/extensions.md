...
The proxy is also the trust boundary in the other direction: it strips browser credential headers and upstream cookies, applies no-store and no-referrer response policy, stamps every upstream request with `X-Chan-Extension-Scope`, and keeps anonymous capability callers, who reach an extension link through the gateway with no session, read-only; the owner and a grantee use the proxy alike. The full header, redirect, and timeout hygiene is in the [configuration reference](config-reference.md).

...
Accepted. Team Work agent tasks will be exposed to agents through the chan
...
Sending a task stores it first, then best-effort wakes a live recipient
terminal with a minimal `poke` line. The poke is only a wake-up signal. Agents
read retained tasks by calling `list_agent_tasks` over MCP, which keeps
...
interface with the volatile store, configured depth, global task ids,
...
`poke` in the terminal means they should call `list_agent_tasks` over MCP.

Team bootstrap exports both `CHAN_TEAM_NAME` and `CHAN_TAB_NAME` into each
member PTY. The advertised MCP proxy command, whether provided by the CLI or the
packaged desktop binary, sends a private prelude line before MCP traffic:
...
`list_agent_tasks` is cursor-based but has no ack state. Without `since_id`,
it returns the currently retained inbox oldest to newest. With `since_id`, it
returns retained tasks where `id > since_id`. The response includes
...
Tasks include `id`, `from`, `to`, `context_path`, and `created_at_unix_ms`.

...
- **Fixes from the gateway review's low findings.** When an admin block, delete, fleet pause or policy change cannot finish draining the control plane, it still answers 502, and the identity log now names each failed controller call with its status and the start of its response. A `DEVSERVER_RETENTION_MINUTES` value too long for the registry sweep to evaluate (beyond about 6,700 years) fails profile startup instead of failing every sweep once a minute, and a value that overflows no longer panics. Desktop entry answers 400 before signing for a navigation path with a backslash, a control character, invalid URI syntax or more than 2,048 bytes, and accepts `://` inside a path or query, which cannot leave the tenant origin. Grant claims and the admin email filter lower-case both the stored and the supplied address in the database, so a grant made to a capitalized non-ASCII address is claimed by its verified form; this needs a gateway database with a UTF-8 locale. A new PAT and its creation audit row commit together, so a failed audit leaves no active token the user never received, and a desktop redemption whose audit write fails still delivers the already-minted PAT once and logs the failure. A proxied request body over the size cap is answered 413 instead of 502. A devserver's owner is let in whether or not the devserver's registry row exists, while anyone else still needs a claimed grant. Admin email resolution reads every page of matches in a stable order, so a user past the first page resolves and one seen on two pages is not reported ambiguous. A desktop entry to someone else's devserver without a grant is refused the same way whether that devserver is online or offline, a rename to a known-taken handle or past the rename limit is refused before the user's tunnels are cut, and the proxy's shutdown drain is bounded at 30 seconds.
...
- **Sharing a devserver is all-or-nothing.** A grantee holds the owner's authority on the devserver: the launcher library, windows, command launcher actions, `root=filesystem` transfers and extension writes. There is no viewer role and no read-only filesystem refusal, because the terminal data plane cannot contain a viewer. Reverse tunnels are the exception and stay owner-only.

- **Reverse tunnels open only from the owner's desktop app.** The two reverse-tunnel legs are refused with 403 for the owner's browser session, for a grantee, and for any session whose client the gateway does not state. This requires the new gateway: an assertion signed before the client field existed reads as an unknown client and never as the desktop, so behind an old gateway a new chan refuses every reverse tunnel, the owner's desktop included.

- **Extension links need a signed-in session.** Anonymous extension capability links are gone. A link is bound to the user who opened the tab, dies within the hour or on revocation, and a leaked link cannot climb out of its extension, because the gateway now refuses any extension request whose path holds a dot segment.

- **`POST /api/tokens` refuses desktop scopes.** A token minted from the dashboard cannot carry `desktop.connect` or `desktop.account`; the request answers 400 `invalid scopes`. Only the desktop authorize flow mints those, and the operator mint still accepts caller-supplied scopes. Tokens already carrying a desktop scope keep working and are not revoked.

- **Closing a window names the terminals it will stop.** Close under Computers > Windows in the launcher, and the Close action in a workspace window whether it is served by the desktop or by a browser, ask once with the window's live terminal count ("1 terminal session in this window will stop.", "This window will close." when there are none, and the generic "Open sessions in this window may stop." for a remote devserver window or when the count cannot be read). The count is read again just before closing and the question is asked again if it moved, or if a newer reading has overtaken the one the card was built from. A control terminal keeps its own warning. The figure comes from `GET /api/library/windows/{window_id}/live-terminals` for the launcher and from the command-capability route beside it for a workspace window, and it is the same host-wide count the `cs window rm` guard reads, so the surfaces cannot disagree. `cs window rm` is unchanged: it refuses a window with live terminals unless given `--force`.

- **`cs tunnel` forwards a connection that closes one direction at a time.** A reverse port forward negotiates half-close, so a client that shuts down its write half still receives what the service sends back instead of having the whole connection torn down. The devserver advertises support in the tunnel trigger and a supporting desktop in the data-leg query; only a leg both ends advertised uses the end-of-stream marker, and any mixed-version pair keeps the previous both-directions-close contract. Binary data after a marker is discarded, an I/O error or a closed channel still ends both directions, and every exit sends a real Close frame under a bounded send.

- **A blocking `cs` request that the user interrupts releases what it held.** `cs terminal survey` and a requester-side `cs session handover` keep their write half open and ask the server to cancel on end-of-file. A queued survey releases its turn in the queue, an open survey cancels its waiter and closes its overlays, and a handover clears its pending slot without moving leadership. Requests that do not ask for it are unchanged in both directions of version skew.

- **Stalled transfers no longer wedge a devserver.** A download or upload whose other end stops making progress fails after five minutes, configurable as `[transfer] stall_timeout_secs` in `server.toml` or through `chan config set server.transfer.stall_timeout_secs`, instead of holding one of the two transfer workers forever; two stalls used to hang every upload and download. Turning a workspace off also ends the `cs` connections opened through it, `cs tunnel` included.

- **A folder or symlink download too large to send is refused before the body starts.** The preflight counts the tar framing as well as the file bytes, so such a download answers 413 naming the encoded size and the ceiling instead of streaming a body that stops part-way. A tree that changes after the preflight is still bounded by the archive writer.

- **Code reports read only what they bound.** Counting a non-UTF-8 text file (UTF-16, Latin-1) and recognizing an extensionless script use the bounded read the scan already made, so a file that grows or is replaced in between is never read in full. A file over 16 MiB stays in the report with its size and language but is never read, so saving and reporting no longer stall while a SQL dump or a large log is re-read on every change. A folder the app cannot read inside a workspace (`lost+found`, `.Trashes`, a Docker volume) no longer breaks the whole report, and a workspace whose scan had to skip entries keeps its report cache, so opening it shows the cached report at once while the usual background rescan refreshes it. The report metadata, `repo_report` over MCP included, carries `skipped_entries` when the scan skipped anything.

- **Devserver sockets refuse a request without its newline.** The desktop handoff and devserver registration sockets refuse a request line missing its final newline ("invalid handoff request: line is not newline-terminated", and the registration twin), so a client whose write failed just before the newline can no longer have a workspace mounted while it reports the request as not sent. Both sockets also drop a client that sends nothing for ten seconds.

- **Share links run one landing flow.** Per-workspace and root share links share one implementation, and a per-workspace link checks the owner before the signed-in caller, as root links already did. Every refusal is still the same 404.

- **An nginx edge in front of the gateway tunnel needs idle timeouts that fit a tunnel.** A registered tunnel is one long-lived `POST /v1/tunnel` whose request body is the devserver-to-proxy direction, and that body is idle whenever nobody is using the devserver, so nginx's default 60-second `client_body_timeout` ends it with no error on either side and a `200` in the access log: a stock edge shows up only as a devserver that re-registers about once a minute forever. Raise `client_body_timeout` on that location, and `grpc_read_timeout` with it, since that is an idle deadline on the same request's upstream leg and expires with a `504`; `grpc_send_timeout` is armed only while a write to the upstream is pending, so it is not an idle deadline here. The gateway's setup guide, the proxy's README and the agent guide each state the requirement, and they also say that a TLS-terminating edge decodes and holds the bearers it forwards, so an edge host should be isolated the way the gateway pod is with token rotation and revocation as the control.

- **The per-user tunnel limit says what it counts.** The tunnel server's refusal reads "user <name> reached max concurrent devserver registrations (<n>)" instead of "... concurrent workspaces", with the wire error code unchanged. The chan gateway passes no per-user cap, so it does not produce this refusal.

- **CLI help is complete.** Every visible `chan` and `cs` argument has help text, in `--help` and in `chan dump-skill`; `cs terminal new` documents its positional PATH and when it is refused; and the "a self-managed daemon is already running" error names the real `chan devserver restart` and `stop` commands.

- **Build and release tooling.** For contributors: `make pre-push` fails when `Cargo.lock` is not the lock the Nix `cargoHash` pins were harvested for (`make nix-hash-check`, digest in `packaging/nix/cargo-lock.sha256`; re-pin with `make nix-hash-pin CARGO_HASH=...`, which refuses re-pinning a stale value, a stale value over a malformed digest, and a `cargoHash` equal to the npm hash), and every version bump, release candidates included, harvests the hashes. The gateway workspace's docs build under `-D warnings` in `pre-push` and in CI (`make gateway-doc`), `make gateway-test` runs every gateway binary's unit tests, `make web-check` also runs the web-shared package's checks and tests, and the Windows CI test subset fails closed when a filter matches nothing or an `#[ignore]` hides a case it was meant to run.

- **A workspace whose folder was deleted or replaced says so everywhere.** A mounted workspace whose root directory is removed or swapped for a different one is reported as unavailable with a reason that names the path and the verbs that clear it, instead of reading as healthy while every file request through it answers 404. On Linux and macOS the check compares the directory's identity, so the state clears exactly when the original directory returns to that path and never for a freshly created one; on Windows the check sees only a root that stops answering and cannot tell a replaced directory from the original. `chan ps` shows such a workspace as `degraded` and `chan workspace status` prints the reason; a holder that reports no mount state at all, such as a standalone or desktop serve, still shows `served`. A mounted workspace counts as on whether or not its root answers, matching what a connected devserver already reported, so the power control offers the action that helps. `POST /api/library/workspaces/{id}/on` answers 200 with the workspace's row on every success, healthy or degraded, where it answered 204 with no body, over a dead root too; the refusals that are not a row are unchanged. Both web surfaces list the status and draw its reason, and one classifier decides what a status means so a status added to the wire has no meaning until someone gives it one.

### Removed

- **Outbound workspace attach, the legacy window label families, and sixteen unreachable commands are gone from the desktop app.** The URL-attachment ("outbound") workspace windows, their restore and their Window menu entries are removed; no path in the app could open one, because their only openers were internal commands no window was allowed to call. Older desktop config files still load: their `outbound[]` and `window_configs[]` rows are ignored and not written back. The `terminal-<n>` and `workspace-<hash>-<seq>` window label families are no longer recognized, and `cs window open` given one reports that the window is not open. Sixteen internal commands no window could call were removed with their permissions, and the macOS build no longer pulls `objc2-web-kit`, `objc2-core-foundation` or `semver`, leftovers of a removed PDF module.

### Fixed

- **File operations no longer lose data.** Renaming onto an existing file is refused instead of silently replacing it; a directory copy that fails leaves nothing half-copied; promoting a draft into its own folder is refused instead of deleting it; a failed draft merge rolls back; a copy or move whose destination is inside its source, including through a symlink or a differently-cased name, is refused instead of leaving a stray folder behind; and a trashed file goes back where it was when its metadata write fails.

- **Uploads and saves no longer clobber each other.** A save whose file changed on disk while its body was uploading gets a conflict instead of overwriting the external edit; two attachments uploaded with the same name at once both survive, the second under a numbered name; a long upload or copy keeps its attribution and echo suppression; and a metadata import or index reset refused as busy no longer closes open documents and terminals first. Downloads containing symlinks now succeed: the link is stored in the archive as a link and its target is never read, where before such a download was refused or failed part-way for a dangling or large target.

- **Metadata exports take consistent snapshots.** The graph and the search index are snapshotted together, and the write journal and rebuild marker are captured between the two snapshots and archived from those copies, so a write whose graph commit landed before the first snapshot and whose index commit landed after the second leaves a record instead of an archive that imports as silently stale search. An interrupted rebuild is bracketed the same way, and an older chan reads the archive as it always did.

- **Workspaces stay available under failure.** A whiteboard open on a stalled network mount is no longer announced to every attached client as deleted: the existence probe read every transport error as "absent", so a mount answering `ENOTCONN`, `ESTALE` or `EIO` made the reconciler fan a removal to each attachment, drop the compare-and-set token and stop the flush clock. Beyond that, a semantic search during a rebuild can no longer deadlock a workspace; a failed index config write no longer leaves the running index out of step with disk; one damaged vector cache file no longer takes down text search; two processes opening a workspace at once can no longer both hold its writer lock; opening a workspace under descriptor pressure fails after three seconds with a retryable 503 instead of hanging; a hung network mount no longer stalls every other workspace while one mounts; and `cs search` and saving the excluded-directories list no longer stall other workspaces while they run.

- **Workspace lifecycle is reliable.** Turning a workspace off no longer waits on, or leaves the workspace locked by, a connected MCP agent, and is remembered even if the request was interrupted, so it does not come back on after a restart; an interrupted teardown no longer leaves a workspace spinning "closing"; turning one back on right after turning it off waits up to a second for the previous handle instead of reporting "still releasing" at once; a failed remove or an interrupted start shows an error you can retry instead of spinning forever; and concurrent window changes can no longer lose a window or corrupt the saved window list. A window that leaves a shared session is always reaped, so leadership moves.

- **A workspace whose startup report scan was stopped still gets a fresh report.** The open-time refresh of a workspace's persisted code report is recovery state: a recovery pass cannot complete until its claimant has refreshed the report, so a stopped startup worker or a failed scan leaves the same pass queued for the next claimant instead of leaving a stale report in place. A refresh that keeps failing gets one immediate retry and then settles the pass while keeping the obligation for a later independent pass.

- **Search keeps plain-text files right.** Opening a workspace no longer re-indexes every `.txt` file each time; a `.txt` deleted while the app was closed disappears from search; links from notes to a `.txt` file no longer vanish from backlinks; and an imported contact whose email address carries an underscore (`first_last@example.com`) is searchable as the whole address, where the note's Markdown escape truncated it. Renaming a folder no longer fails because an unrelated large sibling exhausted the listing budget, since the mapping walk is confined to the source subtree, and a search over non-ASCII text no longer panics.

- **Missing files are a 404 on every operating system.** Opening a file, folder or draft that no longer exists answers 404 on Windows too, where it answered 500 and the app's missing-file recovery never engaged; creating a draft whose name is taken is a 409 and retries the next name; and moving a folder into its own subfolder is refused with 400 instead of failing with a 500 that left an empty folder behind inside the source.

- **`chan upgrade` on Windows no longer leaves the install path empty when the swap fails.** The replacement renames the staged binary to a probe name in the executable's own directory before it moves the running image, so a sharing violation on the staged file fails while the executable is still in place. A Ctrl-C listener is held across the whole replacement, so a console Ctrl-C while the path is empty no longer skips the rollback. The staged binary can only be built in the running executable's own directory and is refused anywhere else before the first rename, so every replacement rename is a same-directory move, and every rename in the sequence, the rollback included, makes at most ten attempts on a Windows sharing violation, with nine waits totalling 1.625 seconds, and returns any other error at once. Ctrl-Break, closing the console, a logoff or shutdown and any external kill still end the process mid-swap, leaving the previous executable at the backup name.

- **The CLI finds the workspace you meant.** `--workspace` and running from inside a workspace resolve even through a moved-and-symlinked root or a Windows `\\?\` path; `chan workspace status` no longer registers the path or fails with "locked" for a workspace that is being served, it reports who serves it; and `chan serve` waits up to 75 seconds for a devserver that is still mounting, printing a note, and refuses to start a second standalone server if the devserver never answers instead of racing it. When chan hands a workspace to a running devserver and that devserver dies after taking the request, `chan serve` stops with an error pointing at `chan ps` and `--standalone` instead of opening the workspace itself while the devserver remembers it as on and re-mounts it on restart.

- **`cs` on macOS no longer fails fast commands with "Socket is not connected".** A quick control request such as `cs window list` could exit 1 with "closing control request: Socket is not connected (os error 57)" when the server answered and closed the socket before `cs` half-closed its side, although the reply had arrived; `cs` now reads the reply. A `cs` command, or `chan close`, whose server closes without answering says "the server closed the control socket before answering" instead of a JSON decoding error.

- **The desktop app recovers instead of stalling.** A window whose native build fails is retried instead of silently never appearing; a gateway connect that cannot start sign-in is parked and offers Connect again, instead of leaving the card in Connecting with no buttons; adding or turning on a workspace on a slow devserver no longer times out after five seconds, and turning one off on a slow devserver no longer reports a timeout while the off completes; forgetting a workspace with live terminals over a gateway reports the terminals and can be forced, where it failed with a raw HTTP 409 and could not be forced; a gateway whose stored credentials cannot be read shows the error and a notice instead of silently doing nothing; and the AppImage no longer overwrites `chan` through a `cs` link, which made both names run as `cs` and broke the devserver unit.

- **A WebSocket through the gateway no longer wedges when one side stops reading.** Each relay direction owns its task, with a policy monitor holding the shared idle deadline, revocation and expiry, so backpressure toward one peer no longer stops reads from the other or keeps the idle window from firing. A direction that ends normally leaves its peer running so frames already read still drain, and every final Close send is bounded. A WebSocket that cannot reach its devserver closes with 1011 `upstream timed out` after the idle window instead of hanging.

- **Gateway availability no longer depends on one tunnel behaving.** A transient `accept()` failure no longer takes down a node's tunnel listener or the fleet's controller; a panic in one proxy's control session no longer stops the controller; controller writes to a proxy that stops reading time out after 15 seconds and release the connection; a tunnel row, lease refresh or admission request the controller cannot verify kills or refuses that one tunnel instead of ending the proxy's control session, which used to evict every tunnel on the node every grace period; the proxy refuses mis-bound and unverifiable refreshed leases itself; a lease that expires, or that a kill finds already gone, no longer makes the controller resync the whole proxy; a lease refresh that crosses a kill no longer ends the proxy's control session; a proxy that reaches its per-node tunnel limit or the fleet memory budget refuses the one extra tunnel instead of dropping its control session; one extra browser sign-in at fleet capacity is refused on its own instead of dropping the proxy node's control session; and a refused tunnel dial no longer leaks a task and a socket. A heartbeat no longer rebuilds the fleet inventory at all: it materializes a view only for the rows that actually changed.

- **A devserver whose gateway row was swept can be reached again by its owner.** The access check reads that row for the owner as well as for grantees, so a row the offline sweeper removed locked the owner out of their own live devserver. The row is now ensured on every validated dial and every lease refresh, label-less when the dial announces no name, so it comes back within a refresh interval instead of waiting for a display-name announcement that runs once per dial and only when the client sends one. The ensure runs on its own task, so a slow profile service cannot turn a dial or a lease refresh into a failure.

- **Gateway accounts behave under revocation.** An absurd token lifetime is refused with 400 instead of crashing the request; a blocked or deleted account can no longer open a shared devserver through an old sign-in cookie; revoking a user's access schedules the same durable follow-up cut as a block and gets its own retry window rather than whatever the first cut left; an account deletion that cannot confirm its tunnel cut keeps retrying instead of being silently dropped, and the account can no longer be unblocked in the meantime; revoking a user's sessions also ends any sign-in session that was never recorded in the session index; an admin block on an account pending deletion keeps its "account deletion pending" reason; and deleting an account an administrator already blocked keeps the administrator's reason, including a null one, and the original block timestamp.

- **The gateway profile package installs beside its own release.** The profile `.deb` declared a dependency on `chan-gateway-identity (= 0.78.0-1)` in every release since that pin went stale, so an operator following the README and installing the profile package beside the same release's identity package was refused by apt and dpkg with an unmet dependency. The pin now moves with the release, and `make gateway-version-pin-check`, which the Linux gate runs, asserts it against the gateway workspace version.

- **Agents no longer keep a workspace alive.** A connected MCP agent no longer keeps a workspace or its lock alive after it is turned off, and its tool calls during a storage reset or metadata import wait and then run against the new workspace instead of breaking the reset.

- **Model bundles.** `make models` no longer keeps a truncated embedding bundle as current when its final write fails.

- **Small API details.** A terminal upload whose body fails answers 500 naming the failure ("request body read failed: ..." or "request body ended before completion"); the `terminal_broadcast` window command from `POST /api/terminals/{session}/broadcast` reaches only the owning window's `/ws` socket, where it went to every socket and the app ignored it elsewhere; and the systemd unit accepts chan's own `devserver run` verb.

### Security

- **Attachment downloads say do not sniff, and active content is sandboxed on every download route.** `x-content-type-options: nosniff` is unconditional on workspace file downloads, workspace directory archives, terminal file and archive downloads and the download of a file held in a live editing session, and `content-security-policy: sandbox` is set on all of them wherever the path is active content. Before, the two headers were set together and only for active content, so an ordinary binary, a raster image and every archive went out with neither.

- **The gateway refuses an extension request whose path holds a dot segment.** The proxy forwarded the remainder of an extension link's path to the devserver as it arrived, dot segments included, so keeping a leaked bound path inside its extension rested on the devserver never normalizing before routing. Any request on an extension path that holds a `.` or `..` segment, however it is spelled, is now answered 404 by the proxy: the check splits on `/` and `\`, then percent-decodes and looks again, up to four rounds, so `..%5c` and a double-encoded `%252e%252e` are refused along with the plain form, while `...`, `.hidden` and `a..b` remain ordinary segments and pass.

- **A gateway-served window cannot use the desktop as a reachability oracle.** The grant minted for a library window served from a gateway tenant origin no longer carries the URL probe command, so a script at that origin cannot ask the desktop to issue a GET from the desktop's own network context, with that origin's cookies attached, and read back the status. The bundled local connecting screen, the only caller, still reaches it.

- **The tunnel token is out of the proxy's memory before the tunnel runs.** The accept, validate and register steps hold the dial's bearer and the request head as their own locals and drop both before the connection driver starts, where both were previously held for the life of the tunnel. The client marks the dial's `Authorization` value sensitive, so it is sent as a literal never indexed and an intermediary that re-encodes the request must forward it the same way.

- **Local hardening in chan-server.** The `t=` bearer is no longer written to debug request logs; `chan __mcp-proxy` and the desktop handoff clients connect only to a socket this user owns, not a symlink or another user's socket in `/tmp`; the systemd restart manifest, which carries terminal replay bytes, is never published readable by others; boot cleanup no longer signals a pid from another boot or another process lifetime; the desktop config and the daemon log file are created owner-only; and a tunnel caller whose assertion names no user is refused before any router runs.

- **Path and limit fixes from the review.** Every paged filesystem-graph cursor frame is validated before the walk, the archive preflight goes through the workspace facade, the content search limit is clamped, the client-selected filesystem root is gated on tunnel ownership, the profile bearer tier is derived from the request path rather than trusted from the caller, a joining proxy streams its snapshot past the frame limiter instead of being throttled into failure, the tunnel server budgets substreams instead of killing the tunnel, and a refused dial frees its in-flight slot.

- **Downloads cannot escape the Downloads folder on Windows.** A download whose name carries a drive letter or colon (`C:evil.dll`, `report.pdf:x`) can no longer be saved outside Downloads, and Windows device names such as `CON` or `nul.txt` are made safe. The download size cap is no longer silently skipped for a file whose path contains `api/fs`.

## [v0.98.0] - 2026-08-25

v0.98.0 makes the editor's table grid keep every column the source has, ends `chan serve PATH` with a window on every route, retires the `cs` link card now that every supported install creates the alias, teaches a new slide deck how to add a slide, and keeps a workspace on a flapping network mount open instead of wedging.

### Changed

- **A new slide deck says how to add a slide.** `New slide deck` seeds `# Slide 1` followed by ``* use `@pagebreak` on empty line to create new slide``, so the answer is on the page instead of only in `chan-shell`'s export help and `EDITOR.md`. The frontmatter block is unchanged, so the draft still routes into the slides layout, and the caret still lands at the end of the heading. The bullet is not itself a page break: `PAGE_BREAK_RE` matches a line only when its non-whitespace content is `@pagebreak` or an `<hr>` carrying `class="chan-page-break"`, and this line carries `* use` and prose besides. The backticks also keep the `@` from opening the contact bubble as the caret crosses it, because the marker parses as inline code and contact detection skips inline-code ranges. The seed's tests pin the frontmatter block and the body by exact equality rather than by prefix, so an extra line below the heading cannot drift in unnoticed.

### Removed

- **The `cs` link card is gone from pre-flight.** A new workspace comes up without the bottom-right card offering to put `cs` on `$PATH`, because every supported install now creates it: `install.sh` links it, `install.ps1` writes `cs.cmd` and a Git Bash shim, the deb, rpm, AUR, Homebrew and Nix packages ship a symlink beside `chan`, and chan-desktop writes and self-heals `~/.local/bin/{chan,cs}` on every launch. The card fired only where the automation had not run, which in practice meant a developer running out of `target/debug` being handed a shell command for a name they could already reach. `POST /api/preflight/cs-link` and the `cs_link` and `cs_dismissed` fields on the pre-flight snapshot are removed with it, along with the `cs_dismissed` editor preference; the preferences loader ignores unknown keys, so a library whose `preferences.toml` still carries `cs_dismissed = true` loads unchanged and drops the key on its next save. `editor.cs_dismissed` was also the only read-only key `chan config` knew, so every scalar leaf it exposes is now settable, with `editor.shortcuts` remaining the one collection exception. The pre-flight lock, its phase reporting, and the Dashboard onboarding nudge are untouched, and `chan --help` still carries the manual `ln -s` line under EXAMPLES.

### Fixed

- **The editor's table grid keeps the columns the source has.** A GFM header row whose cells are empty renders as a grid instead of raw pipe-and-dash text, and a row with one empty cell among labelled ones no longer collapses that column and shifts every later cell left. `@lezer/markdown` emits no `TableCell` node for an empty cell, so the widget reads each row from its `TableDelimiter` positions and takes the cell text from the source between them: every segment between two adjacent delimiters is a cell even when it is blank, while a segment before the first delimiter or after the last one is a cell only when it carries text. That is what separates `| | |`, which is two empty cells, from a trailing `A | `, which is one. An escaped `\|` produces no delimiter and stays inside its cell. Rows with no outer pipes, escaped pipes, and ragged short and long body rows render as they did before. The empty and whitespace-only headers, the empty cell in first, middle and last position, and the escaped-pipe row are pinned against what `renderMarkdown` produces for the same source, so the editor and the PDF, slide and copy-as-HTML paths cannot drift apart again; the outer-pipe-free and ragged rows are pinned to their existing widths instead, because the export normalizes a ragged row out to the header width and asserting equality there would pin behavior the editor deliberately does not have. Every assertion parks the caret outside the table so selection-intersect suppression cannot mask a regression.

- **A workspace on a flapping network mount stays open instead of wedging.** `RootedFs` surfaces `ENOTCONN` and the other transport errnos instead of collapsing every failure to "does not exist", so the document reconciler stops reporting a live file as deleted when a FUSE or network mount stalls. The devserver probes mounted roots on a timer and reports a workspace as unavailable rather than leaving a green row over a dead mount, and clears that state as soon as the root answers again. Capability handles revalidate in place across a remount whose inode is unchanged, because a retained descriptor is bound to the dead kernel connection and cannot be revived by path. Teardown raises the reindex cancel flag first, so a mount dying mid-index cannot strand the handle holding the writer flock. The errno set is POSIX, so the classifier is unix-only and other platforms keep their existing behavior.

- **`chan serve PATH` ends with a window on every route.** Turning a workspace back on used to restore its saved windows and mint nothing, so the window the command asked for was not among them and focus landed on whichever one the watcher opened last. An explicit open now mints one window after mounting and restoring, and a newly minted workspace window sorts last for its workspace, so it opens last and takes focus. Workspace window ordinals allocate above the family maximum for that reason: a workspace holding Windows 1 and 3 because Window 2 was closed would otherwise number the new window 2 and sort it into the middle. Terminal windows keep their lowest-free numbering. On the devserver route, registering a workspace is now conjunctive: it mounts the workspace and mints one window for it, a repeat registration reuses the prefix and mints another window, and the CLI says it opened a window rather than only that it registered one. A host with no window registry declines the registration before taking the workspace flock, so a request that cannot produce a window fails instead of reporting success. The boot re-serve path still mints nothing, so a workspace whose windows were all closed comes back with none, and the standalone route, `--no-browser`, and the route decision itself are unchanged.

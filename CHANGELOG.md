...
- **chan publishes a FreeBSD amd64 build.** Every release carries `chan-x86_64-unknown-freebsd.tar.gz`, a statically linked binary packaged like the Linux tarballs with `chan`, `LICENSE` and `README.md`, so `https://chan.app/install.sh` installs chan on a stock FreeBSD amd64 machine: the installer recognizes a `uname -s` of `FreeBSD` and downloads through the base system's `fetch` where neither curl nor wget is present. `chan upgrade` resolves the new target and `/dl/cli/latest.json` lists it. FreeBSD arm64 is not published, and both the installer and `chan upgrade` refuse it by name and point at a source build. Reaching that point fixed what FreeBSD exposed in the workspace layer, all of it FreeBSD-scoped and none of it changing Linux, macOS or Windows behavior: the file watcher recovers the creations notify's kqueue backend drops (it names only the first new entry per directory write), the descriptor probe reads the real `RLIMIT_NOFILE` and declines to trust a `/dev/fd` listing that a one-time probe shows is a devfs stub, and two cap-std assumptions that held only on Linux are corrected, so an `O_PATH` directory handle is reopened through `openat(dir, ".")` instead of procfs and a kernel-resolved `ENOTCAPABLE` is classified as the escape it is.

...
The two static linters (`make shell-check`, `make workflow-check`) cover the shell scripts and GitHub workflows that no cargo or npm target reads. `make build-matrix-check` then asserts that the native macOS, Windows, and Linux package jobs plus the distro and container jobs still invoke the repository's real build targets. It is deliberately a wiring test: a Linux hook cannot execute AppKit or produce an NSIS installer, so the authoritative compile/package proof stays on native CI runners. `make nix-hash-check` fails when the root `Cargo.lock` is not the lock the Nix `cargoHash` pins were harvested for (its digest is recorded in `packaging/nix/cargo-lock.sha256`), when the two pins disagree, or when a pin is a placeholder such as `lib.fakeHash`; a lock change needs the hash harvested with `make nix-sdme-check NIX_PACKAGE=chan` on Linux or `make nix-check` where Nix is installed, with the `Cargo.lock` you will pin in the working tree, then pinned with `make nix-hash-pin CARGO_HASH=sha256-...`, or CI's Nix job goes red on `main`. The pin helper refuses the value both derivations already carry when the lock is not the one it was harvested for: that is the `specified:` line a Nix build prints beside `got:`, and `got:` is the value to copy. On a macOS or Windows host neither harvest target runs, and the pre-push hook blocks the push that would let CI's Nix job report the value, so harvest on a Linux host or in a Linux container (a checkout of the branch there runs the same two commands), or ask a maintainer to harvest and pin it. `make nix-hash-contract-check` proves that checker and its pin mode against a throwaway directory first. `scripts/lint-static.sh` downloads both linters at a pinned version, verified against a checksum, into `${XDG_CACHE_HOME:-~/.cache}/chan/lint-tools`, so no system package or root is needed. Only a cold cache needs network; the cache deliberately sits outside `target/` so wiping `target/` or adding a worktree does not force a re-download. Set `CHAN_LINT_TOOLS_DIR` to move it. Severity and the exclude list live in [`.shellcheckrc`](.shellcheckrc), each exclude with its reason.

...
Thanks for the interest. chan is an IDE in a single binary: a terminal emulator and multiplexer plus a workspace manager for any directory on disk. Contributions are welcome.
...
Installing the pre-push hook (`./scripts/install-hooks`) wires the same gate that CI enforces: shellcheck + actionlint + the cross-platform build-matrix contract + the Nix cargoHash check + fmt + clippy `-D warnings` + rustdoc `-D warnings` + cargo test + `cargo build --no-default-features` + the separate gateway workspace (fmt, clippy, rustdoc, database-free tests, release build) + `npm run check` + `npm test` + `npm run build` + a release devserver boot smoke + the native desktop package against the pinned Rust toolchain. Linux produces an AppImage; macOS produces an ad-hoc-signed `.app`. Run it locally before pushing; CI will fail fast otherwise.

The two static linters (`make shell-check`, `make workflow-check`) cover the shell scripts and GitHub workflows that no cargo or npm target reads. `make build-matrix-check` then asserts that the native macOS, Windows, and Linux package jobs plus the distro and container jobs still invoke the repository's real build targets. It is deliberately a wiring test: a Linux hook cannot execute AppKit or produce an NSIS installer, so the authoritative compile/package proof stays on native CI runners. `make nix-hash-check` fails when the root `Cargo.lock` is not the lock the Nix `cargoHash` pins were harvested for (its digest is recorded in `packaging/nix/cargo-lock.sha256`), when the two pins disagree, when a pin is a placeholder such as `lib.fakeHash`, or when a pin is the same file's `npmDeps.hash`; a lock change needs the hash harvested with `make nix-sdme-check NIX_PACKAGE=chan` on Linux or `make nix-check` on a Linux host with Nix installed (both targets are Linux-only), with the `Cargo.lock` you will pin in the working tree, then pinned with `make nix-hash-pin CARGO_HASH=sha256-...`, or CI's Nix job goes red on `main`. The value to copy is the `got:` line of the mismatch whose derivation name ends in `-vendor-staging`; a `-npm-deps` mismatch is `npmDeps.hash`, which a build reports first when `web/package-lock.json` changed too, and is re-pinned by hand in both derivations before a build reaches the cargo one. The pin helper refuses the value both derivations already carry when the digest file does not name the live lock: that is the `specified:` line a Nix build prints beside `got:`. A malformed digest file (a merge or rebase conflict left in it) is refused the same way, with the restore named; only an absent digest file lets that value pin. On a macOS or Windows host neither harvest target runs, and the pre-push hook blocks the push that would let CI's Nix job report the value, so harvest on a Linux host, or in a Linux VM or container that has Nix installed with this working tree mounted (`make nix-check` runs there; `make nix-sdme-check` needs sdme on a Linux host), or ask a maintainer to harvest and pin it. `make nix-hash-contract-check` proves that checker and its pin mode against a throwaway directory first. `scripts/lint-static.sh` downloads both linters at a pinned version, verified against a checksum, into `${XDG_CACHE_HOME:-~/.cache}/chan/lint-tools`, so no system package or root is needed. Only a cold cache needs network; the cache deliberately sits outside `target/` so wiping `target/` or adding a worktree does not force a re-download. Set `CHAN_LINT_TOOLS_DIR` to move it. Severity and the exclude list live in [`.shellcheckrc`](.shellcheckrc), each exclude with its reason.
...
`gateway/` is a separate nested Cargo workspace (the account / sign-in / reverse-proxy surface for chan.app: profile, identity, devserver-proxy, devserver-control + devserver-control-proto, admin, gateway-common). It is NOT a member of the root workspace, so a root `cargo build` never touches it and the core build stays free of Postgres and the sqlx/oauth2 stack. `make pre-push` explicitly crosses that boundary with `make gateway-build`, a Postgres-free release compile of every gateway service. The gateway's own integration gate (`.github/workflows/gateway-ci.yml`) adds Postgres-backed tests and builds all four OCI service images. The gateway ships only linux amd64/arm64 `.deb` packages, versioned in lockstep with the root (bump `gateway/Cargo.toml` in the same commit as the root version).

Unlike the core, the gateway is Postgres-backed. Its integration tests (`profile`, `identity`) create a throwaway schema per test under `TEST_DATABASE_URL`. The supported, reproducible setup runs Postgres in an `sdme` container (the same way CI does) and works on both Linux and macOS; see [`gateway/docs/dev-setup.md`](gateway/docs/dev-setup.md). Any Postgres with a `chan_gateway_test` database also works:
